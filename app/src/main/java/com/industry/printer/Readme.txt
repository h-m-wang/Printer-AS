...
==================
在control2_frame.xml, control3_frame.xml，control4_frame.xml中tv_start控件后后面添加一个ble_state控件。该控件为标准页面（control_frame.xml）中显示蓝牙状态的控件，由于其余控件与标准控件共享代码空间，因此，如果在这几个特殊页面中不追加该控件的话，会导致找不到控件的异常而死机
