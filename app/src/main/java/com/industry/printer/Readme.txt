2026-9-8 260908-209300001
==================
1. 取消DataTransferThread类中的setDotCount函数中从MessageTask中读取点数设置给DataTask的操作，以防止DataTask在第一次生成print.bin时不再统计点数的问题
2. 在DataTask类rebuildBuffer函数中，取消对32x3-32x7打印头在设置偏移和镜像时的限制

2026-9-8 260908-209201001
==================
1. 32x2 - 32x7打印头修改设置内容：
                reverseEnable = true;
                shiftEnable = true;
                mirrorEnable = true;
                rotateAble = true;
...
2026-9-7 260907-209101001
==================
1. 修改108mm的scaleW，以便达到等比放大，避免图片变形
2. 修改NativeGraphicJni中的bug，导致贴vbin时黑屏
3. 修改编辑图片变量时读取和显示图片的方法，提高显示及UI操作的反应及时性

2026-9-2 260902-209001001
==================
1. 修改refreshVariable函数中的bug
2. 修改compareRTCvSystemTime函数中，A133的情况下，为判断返回值（I2C出错的话，会返回null）
3. Hp22mm.java的初始化中，将开始守护线程及开始加压的处理提前到PD初始化之前，以便有不插墨盒需要加压的需求

2026-9-2 260902-208901001
==================
hp22mm.so的守护线程中，取消读温度的操作（怀疑这个操作导致打印头断电）

2026-9-2 260902-208800001
2026-9-2 260902-208801001
==================
修改动态内容（DT，动态二维码，动态图片及超文本中的DT）的生成方法:
(1) 由原来的生成全高图片，修改为生成正好高度的图片
(2) 由原来通过BinInfo类实现二值化，然后贴到目标数据区的做法，修改为直接调用NativeGraphicJni中的PasteBmp2Bin函数来直接实现
(3) NativeGraphicJni中追加PasteBmp2Bin函数，用来直接对正好高度的图片进行二值化，并且直接贴到目标数据区的对应位置
*文中注释内容采用2026-9-1日期标识

2026-8-30 260830-208700001
==================
修改NativeGraphicJni当中的BinarizeBmp函数的判断标准，不能使用非黑即白，因为大字机生成的Bitmap可能颜色是深灰，所以，要使用非白即黑来判断

2026-8-30 260830-208701001
==================
1. 取消260828-208601001的修改1，虽然PrnComd的设置，使得ControlTabActivity出现勉强的跳转，但是似乎并不引起误动作，所以恢复，这样可以返回1111-ok是附带全部命令
2. 在PCCommandManager类中修改getInstance函数，如果mInstance不为空，则先关闭后在创建新的，这么做的理由是，当Mipi屏时，屏幕会在MainActivity中进行一次旋转，导致ControlTabActivity的onActivityCreated被创建两次，如果不修改，可能会将网络命令发送给第一次创建的控件中，而这个控件已经被废弃了

2026-8-28 260828-208601001
==================
1. 取消PCCommandHandler类中对PrnComd设置100的操作，这个标识的设置会导致在ControlTabActivity的Handler中，处理MESSAGE_OPEN_MSG_SUCCESS事件时，即启动MESSAGE_PRINT_CHECK_UID，又启动MESSAGE_PRINT_START消息，并且两者相差时间很短，
...
2. 在ControlTabActivity类的sendToRemote函数中，增加是否在UI线程的判断，如果在UI线程，则启动线程来发送网络信息，否则会失败

2026-8-27 260827-208501001
2026-8-27 260827-208401001
==================
对于260826-208201001的修改2，修改bug

2026-8-26 260826-208301001
==================
108mm中墨水量
46355×0.7＝32448.5
改成
...
2026-8-26 260826-208200001
2026-8-26 260826-208201001
==================
1. 恢复108mm头的情况下，apk不插值的处理
2. 增加开始打印时对于当前系统设置的打印头与tlk文件中设置的打印头一致性检查，如果不一致，则禁止打印，否则可能会出现系统崩溃的问题
3. 修改NativeGraphicJni，目的是提高处理效率，具体修改内容见cpp文件的说明
4. 修改BinInfo和DataTask类的响应部分，配合jni的修改，提高处理效率
【说明】3，4点的修改统一使用2026-8-19日期作为标签在代码中进行注释标记

2026-8-24 260825-208100001
==================
修改260824-208000001的bug

2026-8-24 260824-208000001
==================
修改260812-207701001的bug

2026-8-24 260824-207900001
==================
增加非DATA_SOURCE_BIN时也支持重置的操作，但是再下发这个1200命令之前，需要保证需要的新DT数据要提前下发好

2026-8-17 260817-207801001-VirtualPrint
==================
1. 【临时版本】虚拟打印
2. 修改NativeGraphicJni.cpp。增加一个以Java的Bitmap为参数的二值化函数BinarizeBmp，这个通过引入这个参数，可以直接访问数据区（Pixels）而不需要通过getPixels函数获取，因为这个函数会复制数据，导致时间和内存的开销
3. 在BinFromBitmap.java类中，暂时将extract函数修改为extract1（这个函数是使用getPixels函数获得点阵数据的），重新设置一个直接将Bitmap传递给BinarizeBmp函数的extract函数。经过测试，这个方法由于不再使用getPixels函数调用，减少了这部分开销，节省用时三分之一

2026-8-12 260812-207701001
==================
FW升级， 增加GW_xxxxx文件升级 ，
1 按钮和以前一样，
2 . GW_xxxxx.bin ，GW_xxxxx.txt 还是放在   FWupdade 文件夹
3. 按FPGA升级键，  先看是否 FW_XXXX 和 GW_xxxx 同时存在， 是， 报失败
4. 如果只有 FW_XXXX,  按以前执行
5. 如果只有GW_xxxx,    从0x080000 开始 擦除，写入，校验。
（需要ko4045以上版本的配合）

2026-8-11 260811-207601001
==================
修改hp22mm.so的设置状态的逻辑，将RunningState打印头在monitorThread的开始处设置为STATE_VALID，否则，由于增加了超时时PD重新上电的逻辑，之前的STATE_INVALID需要等到IDS加压完成才能够被更新
        RunningState[PEN0_STATE] = STATE_VALID;
        RunningState[PEN1_STATE] = STATE_VALID;

2026-8-11 260811-207501001
==================
增加log输出，测试PD重新上电后，初始化成功仍然报警的问题

2026-8-11 260811-207401001
==================
暂时恢复108mm的情况下，apk做插值操作

2026-8-10 260810-207301001
==================
等待时间 2000ms -> 2500ms；并且增加R10恢复0后再等待5秒（再加上正常循环中的等待2秒，一共等待7秒）

2026-8-10 260810-207201001
==================
等待时间 200ms -> 2000ms

2026-8-10 260810-207101001
==================
1. 修改hp22mm.so（版本号: 1.0.218），当Uart发生超时的时候，借用getErrorString，向apk返回“UART_ERROR_TIMEOUT”字串，通知apk发生超时
2. apk读取到“UART_ERROR_TIMEOUT”字串后，生成R10=1的参数，并且下发；等待200ms后，再次生成R10=0的参数，并且下发，然后等待2秒后重新执行初始化流程

2026-8-7 260807-207001001
==================
public static final int PIXELS_PER_MM = 12
修改为
public static final float PIXELS_PER_MM = (152/12.7f)

2026-8-7 260807-206901001
2026-8-7 260807-206801001
==================
修改变量属性编辑窗的高度输入方法（ObjectInfoDialog.java）。由原来的全部通过弹出对话窗选择预先准备的值的方法，修改为大字机保留原对话窗选择预设值的方法，hp头直接输入数值。主要修改内容：
1. obj_info_base.xml
   修改highEdit控件为EditText
2. ObjectInfoDialog类
   修改根据打印头类型进行切换显示及对事件的响应
3. MessageObject类
   getRealFontsize函数中，对于hp头的输入内容进行限高，最大值不得超过12.66666666666667，对应于点数152点
4. MessageObject类
   getDisplayFs函数，限制返回数值的有效位数为小数点后1位

2026-8-6 260806-206701001
==================
在变量属性对话窗中，增加一个Line Pos的项目（位于高度与内容中间），达到的效果是 如果输入的是 n/m(n,m为纯数字)，则该控件的高设为全高的1/m, 纵向起始位置设为全高的 n/m 的位置

2026-8-5 260805-206601001
==================
修改编辑页面显示的逻辑，目的是充分利用空间显示信息内容。当为7寸屏（1024*600）时，横纵放大2倍显示；当为mipi屏时（1920*1200）时，横纵放大4倍显示，这样使得屏幕不至于空白区域太多。但是内部的坐标体系仍然是高度为152像素

2026-8-4 260804-206501001
==================
调整MIPI高清屏的显示问题：
1. 更新mipi专用的img，屏幕密度调整为320（传统的img为160）
2. 更换drawable-hdpi中的一些图标，因为这里的图标没有更新过，mipi屏的apk会使用这里的图标
3. 增加values-1920x1200，将主页预览区的高度设置为222dp（3.5寸的为75dp，7寸的为194dp），以保证预览图能够纵向填满预览区
4. 增加drawable-1920x1200，将编辑预览区的高度修改为724px，编辑区仍然为152px，这样使得编辑预览区能够纵向填满编辑区（白色区域）
5. 调整日期时间的字体大小，22sp -> 18sp，否则mipi屏时显示不全。调整版本号显示字体大小，25sp -> 24sp

2026-8-3 260803-206401001
==================
取消2026-4-30，临时在108MM的信息后部追加72列的空格的修改

2026-8-3 260803-206301001
==================
恢复apk不再插值的处理

2026-7-31 260731-206201001
==================
IInkDevice.java接口类中，追加判断是否初始化完成的判断函数，在初始化完成前禁止一些如点击开始打印的操作

2026-7-30 260730-206101001
==================
1. 取消260725-206001001的临时修改
2. 修改在CalendarDialog对话窗升级apk的实现机制，原来通过提示窗显示进度的方法，由于是在对话窗中再次显示提示窗，有显示不出来的情况，修改为在对话窗内部显示“转轮+正在升级的提示”
3. 修改在ControlTabActivity中，MESSAGE_PRINT_START处理中，当img是22mm时，选择了22mm及108mm以外的打印头，或者img是22mm以外时，打印头选择了22mm或者108mm是报错，不能打印
4. 修改MessageTask的dealDot函数中的bug

2026-7-25 260725-206001001
==================
【临时版本】R23 = C92，R24 = C93

2026-7-23 260723-205900001
==================
在BaseObject的floatToFormatString函数中，使用String.format(Locale.US, format, f)作浮点数转换，以避免俄语时将小数点转换为逗号的问题

2026-7-23 260723-205800001
==================
临时放开获取设备IP地址的log输出，确认一下有线网络的地址为什么或得不到

2026-7-22 260722-205700001
==================
超时门槛修改为1800ms，PC端的时间间隔修改为1000ms，这样节奏比较从容，否则原来的200ms，容易造成信息积压，导致发生一次延时后，后续无法恢复

2026-7-22 260722-205601001
==================
改进260721-205500001的测试内容

2026-7-21 260721-205500001
2026-7-21 260721-205501001
==================
1. (2026-7-20) 防止正在打印或者清洗的时候升级FPGA。主要操作内容
    1.1 在TestGpioPinsNew类中，启动升级FPGA的时候，检查是否处于打印或者清洗状态，如果是，提示停止
    1.2 在FpgaGpioOperation类中，下发数据与FPGA固件升级的处理进行互斥
2. 在PCCommandHandler类中增加测试网络接收延时的的操作，前提假设是PC端每个小于200ms的时间间隔发送数据，测试apk接收到数据的间隔时间，如果大于200ms则记录一次，总集数次数最近的100次
   在TestMain的测试列表最后一项启动该测试，启动时显示“Started”的提示窗，再次点击后停止，在对话窗中显示测试结果。显示格式未：
    hh:mm:ss <nnn>ms

================= 2026-7-17 协议整理 ===================
//     协议１：　禁止GPIO　
//     协议２：　　
//            0x01(InPin-1)：是打印开始停止
//            其他禁止
//     协议３：
//            0x01(InPin-1)：方向切换
//            其他禁止
//     协议４：
//            0x01(InPin-1)：是打印“开始／停止”控制位。其中，打印开始停止实在apk里面处理的，方向控制是在img里面控制的
//            0x02(InPin-2)：是计数器清零，包括RTC的数据和正在打印的数据
//            0x04(InPin-3)：方向切换
//            0x08(InPin-4)：空
//            0xF0段(InPin-5 - InPin-8)，即0x10 - 0xF0)为打印文件的文件名（数字形式，1-15）
//     协议５：
//            0x01(InPin-1)：方向切换
//            其他禁止
//     协议６：
//            0x01(InPin-1)：打印“开始／停止”控制位。
//            0x02(InPin-2)：是计数器清零，包括RTC的数据和正在打印的数据
//            0x04(InPin-3)：方向切换。方向控制是在img里面控制的
//            0x08(InPin-4)：空


//            0x10(InPin-5)：墨位低（Output1输出，弹窗）
//            0x20(InPin-6)：溶剂低（Output1输出，弹窗）
//            0x40(InPin-7)：无功能
//            0x80(InPin-8)：低有效，常态高。IN-8=0时，喷码机在等待打印（及正在打印的）状态下清洗一次
//     协议７：
//              当细分计数器到达本轮终点的时候， 比如60细分， 到了60次，报警灯亮30s (直接在计数器变量中控制)

通用out逻辑：

out1  ：  PI8       打印状态，  输出 ，  一般接报警灯绿灯
out2  ：  PB11    有错误时输出，       一般接报警灯红灯
out3  ：  PG4     与扬声器联动，         一般接报警灯黄灯

    标准版InPins："PG0", "PI5", "PI6", "PE7", "PE8", "PE9", "PE10", "PE11"
    标准版OutPins："PI8(对应非SPI版本时PH7)", "PB11(绿灯)", "PG4(Beep)", "PH26", "PH27", "PE4", "PE5", ""
    hp22mm版InPins："PI7", "PG1", "PH26", "PG2", "PG8", "PG9", "PE8", "PE9"
    hp22mm版OutPins："PI8(对应非SPI版本时PH7)", "PB11(绿灯)", "PG4(Beep)", "PI9", "PG0", "", "", ""

2026-7-15 260715-205401001
==================
1. 在260714-205201001的基础上，在Hp22mm类中，强制选择IDS0，C77对话窗强制显示IDS1备选
2. monitorThread当中检测气压值的地方，修改为0.5个气压，否则会经常报警

2026-7-15 260715-205301001
==================
hp22mm的service.c中，PAIRING_RESPONSE_TIMEOUT，5000 -> 10000

2026-7-14 260714-205201001
==================
在260714-205101001版本固定了IDS的基础上，修改显示，当前选择的是对应于底层IDS0，界面显示为IDS1

2026-7-14 260714-205101001
2026-7-14 260714-205001001
==================
在Hp22mmNozzleSelectDialog类中，固定IDS的值为不可选。缺省内容为IDS2，Pen1和Slot1

2026-7-14 260714-204901001
==================
将Hp22mm.java类中的initHp22mm函数中的启动守护线程和开始加压的操作后移

2026-7-13 260713-204801001
==================
修改22mm的最大墨水量计算公式，由
...
2026-7-13 260713-204701001
==================
取消260711-204501001增加的初始化次数显示的机制

2026-7-13 260713-204601001
==================
1. 取消Hp22mmSCManager类中关闭rfid发射信号的功能，因为这会导致访问tty7，而22mm的情况下，tty7用来访问IDS/PD的MCU，会导致本来欲发送给RFID的命令而发送给IDS/PD，可能会导致起不在相应其它的命令而出现超时的错误
2. 在Hp22mmSCManager类启用mLibInited，目的是不要执行第二次初始化，因为第一次初始化就已经建立起来了无限循环的监视体系，第二次再入的话，会建立起来多余监视体系

2026-7-11 260711-204501001
==================
临时在Hp22mmSCManager的初始化流程中，增加一个显示初始化次数的功能，借用显示错误的地方会弹窗

2026-7-11 260711-204401001
==================
hp22mm.so取消1.0.214的超时时重复尝试1000次的功能，恢复原来的样式 相当于，恢复到1.0.211(260711-204001001)

2026-7-11 260711-204301001
==================
在service.c的service_execute函数中，增加超时后，再次下发命令的尝试，最多尝试1000次

2026-7-11 260711-204201001
==================
1. 取消260711-204101001的修改，恢复uart.c中超时后尝试三次的操作
2. 在service.c的service_execute函数中，增加超时后，再次下发命令的尝试，最多尝试3次

2026-7-11 260711-204101001
==================
【临时版本】当发生TIMEOUT时，循环等待，一直到TIMEOUT消失，不设重新尝试次数

2026-7-11 260711-204001001
2026-7-10 260710-203901001
==================
1. hp22mm的so中，在uart.c的uart_recv函数中，增加出现超时错误时，重复尝试3次的功能
   在uart.c的uart_select_mux函数中，如果切换PE4不成功，尝试3次，每次尝试切换后等待10ms
2. 在DataTransferThread类中的PrintTask中，取消1分钟后的自动更新，因为需要自动更新的内容已经通过mDataTask.get(index()).contentChanged()实现了
	// mHandler.sendEmptyMessageDelayed(MESSAGE_DATA_UPDATE, MESSAGE_EXCEED_TIMEOUT);

2026-7-8 260708-203800001
2026-7-8 260708-203801001
==================
在测试页面（TestMain.java）的最后追加一个测试项目（Read Write Test/U盘读写测试），具体动作为：
1. 点击后开始读写测试，显示提示对话窗
   Read/Write xxx times
   xxx error occurred.
   第一行提示读写测试的总次数，第二行提示发生错误的次数。错误包括：读写时系统检测出异常发生和读写内容不一致
2. 点击退出按键结束测试
3. 读写内容为100个字符，每100ms读写一次
4. 读写位置为U盘的根目录，生成一个名为wrtest.txt的文件，读写操作在该文件上进行

2026-7-6 260706-203701001
==================
【临时版本】apk启动后，持续显示白屏

2026-7-3 260703-203601001
==================
108mm的情况下，最大锁值在46355的基础上下调30%，46355×0.7＝32448.5

2026-7-3 260703-203501001
==================
暂时取消260623-202800001修改3的取消插值处理的修改，暂时恢复apk插值，等待FPGA的修改完成
（似乎版本号没有修改，还是260630-203400001）

2026-6-30 260630-203401001
==================
增加第三方墨袋墨盒的编程功能。具体修改内容：
1. hp22mm库
   增加六个接口函数
       读取PD的相关数据项的值： readPDProgramingFields();
       读取IDS的相关数据项的值：readIDSProgramingFields();
       修改PD的相关数据项的值： writePDProgramingFields(int l2, int l3, int l4, int l5, int rev);
       修改IDS的相关数据项的值：writeIDSProgramingFields(int l2, int l3, int l4, int l5, int rev);
       锁定PD的相关数据项区域： lockPDProgramingFields();
       锁定IDS的相关数据项区域：lockIDSProgramingFields();
2. 在apk中的测试页面，增加测试项目，在测试页面的最下部，增加以
    [PROG] 开头的测试项目，分别对应于上述操作内容
3. 修改数据项的内容固定值为
    L2 – Ink Formulator 值为2
    L3 – Ink Vehicle 值为1
    L4 – Ink Family 值为9
    L5 – Ink Family Member 值为0
    Ink Revision 值为0
    ABSlot的值相同
【注意】写入新值后，需要重启数据才能生效，否则配对可能会失败
【注意】锁定后就不能再次修改了，需要谨慎操作

2026-6-26 260626-203301001
==================
正常版本，没有203100001和203200001版本写IDS及PD的L2-L5的操作，apk正常初始化hp22mm驱动（全部功能）。实际功能与203000001相同

2026-6-26 260626-203201001
==================
借用 getUsedInkVolume 来写IDS和PD的L2-L5信息，不同信息，IDS_L2=9。apk进初始化IDS和PD，不加压，不配对

2026-6-26 260626-203101001
==================
借用 getUsedInkVolume 来写IDS和PD的L2-L5信息，相同信息（10, 1, 12, 13, 14）。apk进初始化IDS和PD，不加压，不配对

2026-6-25 260625-203001001
2026-6-25 260625-202901001
==================
修改22mm的最大墨水量计算公式，由
...
2026-6-23 260623-202801001
==================
1. 取消AndroidManifest.xml中WelcomeActivity和MainActivity中的
            android:screenOrientation="landscape"
   定义，改为在各个Activity的onCreate函数中，根据img的版本号来决定如何旋转
   （原因：MIPI屏是旋转了270度横屏，但是其它的屏是旋转90度横屏（M9是不旋转的横屏），因此，相对应apk要设置的Orientation不同，
       MIPI: reverseLandscape；其它：landscape
2. 在img的生成阶段，mipi专用的img启用8012-8999号段的版本号，apk根据img的版本号是否是该字段来判断img的中了，从而决定旋转角度
3. 恢复2026-5-9版本修改的108mm打印数据的生成方式（不在apk做插值，也不做镜像）
   3.1 PrinterNozzle类中，mHeight = 2560 (原值= 508*4 + 528)
   3.2 PrinterNozzle类中，scaleH = 2560f/152
   3.3 PrinterNozzle类中，ratio = 1.0f * 108.0f / 304
   3.4 BinFromBitmap类中有关插空和生成bin数据的功能
   3.5 DataTask类中的rebuildBuffer函数中，取消108MM的倒置处理
   3.6 BufferRebuilder类中，取消reverseHp108mm的定义和调用
4. 在Hp22mm.java类的getSettings函数中，追加R7，R8和R9的设置（传递给FPGA）
    R7[5:0] = C83
    R8[4:0] = C84
    R9[0] = 0:不倒置；R9[0] = 1:倒置

2026-6-5 260605-202700001
==================
1. 在hp22mm.c中，修改函数 Java_com_UpdatePDFW, Java_com_UpdateFPGAFlash, Java_com_UpdateIDSFW 的接口，将待升级文件的指定由这里固定目录+文件名修改为由apk指定
2. 同步在hp22mm.java中的接口函数，以及TestHp22mm.java中升级PD，FPGA及IDS函数调用处指定文件目录+文件名

2026-6-1 260601-202600001
==================
202500001修改内容基础上，虽然保留从pw.txt读取密码，但在模式8的情况下，设置固定密码tianjie123

2026-5-30 260530-202500001
==================
增加用户模式8，进入编辑页面需要验证密码。密码保存在/sdcard/system/pw.txt文件中，可以指定两个密码，每行为一个密码

2026-5-28 260528-202401001-VirtualPrint
==================
修改260525-202201001版本增加的虚拟打印功能，将打印内容保存到U盘的根目录中，文件名为Print****.bin，****为1000开始每次打印递增

2026-5-26 260526-202301001
==================
1. 在hp22mm.c中，将墨量最大值由固定数值修改为由apk设置
2. 在Hp22mmSCManager中，将最大墨水量修改为可变值，当打印头为22mm的时候，最大值为49600，108mm时，为49600/1.07=46355。通过这个修改，实现实际墨水消耗率（%）与apk中的统计（%）基本一致，并且在统计数字耗尽时，留有一定的存量
3. 取消之前2026-5-22 260522-202101001中的修改内容

2026-5-25 260525-202201001
==================
增加一个虚拟打印模式，当该模式开启以后，忽略硬件IDS，PD的初始化而可以直接开启打印，每次打印的print.bin均输出到存储，以便于从bin文件中确认打印内容，而不知实际输出到打印机硬件

2026-5-22 260522-202101001
2026-5-22 260522-202001001
==================
260521-201701001的修改2，由1.3改为1.4

2026-5-22 260522-201901001
==================
在hp22mm.so中增加一个记录累计墨水消耗量的数值，可以通过22mm的测试页面读取确认

2026-5-21 260521-201801001
==================
修改打开信息对话窗显示可放大的预览图的方法

2026-5-21 260521-201701001
==================
1. 修改PrinterNozzle类中isBigDotType函数中的bug（32x3-32x7不正确）
2. 在BinFromBitmap类的二值化处理后，点数聚敛到头1后，头1的点数再乘以1.3，以使得逻辑计数和物理消耗同步

2026-5-20 260520-201601001
==================
在BinFromBitmap类中，108MM打印头的情况下，二值化完成后，将分散于各个假设头的点数汇总到头1（原来应该的结果）

2026-5-18 260518-201501001
==================
增加预览图片放大显示功能，具体内容如下：
1. 在主页的预览区右下角显示放大镜图标（仅108MM打印头类型时），点击图标进入放大显示页面
2. 页面初始显示满屏，等比例预览图，单指拖动移动图片，双指放大缩小图片，双击复原图片
3. 优先显示1A.bmp图片文件，若无，则显示1.bmp。因此需要显示的大图需要保存在1A.bmp中
4. 图片最大放大4倍，最小不小于边框范围

2026-5-13 260513-201401001
==================
1. dataloading画面长按出现的对话窗中，选择中间按钮清除数据时，保留F1.txt和F2.txt不被清除
2. 编辑页面，108mm的背景修改为分5个区的图
3. 修改apk升级逻辑。当开机时当前apk的版本号与F2的版本号不一致的时候，原来推断为push升级，卡在Dataloading画面，现在修改为此时提示用户重新升级，点击屏幕后执行再升级
4. 修改升级复制文件逻辑，将复制文件时的线程取消，可能flush没有成功的原因就是这个线程复制文件耽误时间了
5. 修改NativeGraphicJni（1.0.13）的二值化处理逻辑，增加不中间填充空白操作的分头统计
     (TarLines == 0 && dot_count == width / head)

2026-5-13 260513-201301001
2026-5-13 260513-201201001
==================
修改DataTransferThread类的getInkThreshold函数的hp22mm时的计算公式，原来的
    mThresHolds[head] = ((mThresHolds[0] + mThresHolds[1]) == 0 ? 0 : mThresHolds[0] * mThresHolds[1] / (mThresHolds[0] + mThresHolds[1]));
时，如果mThresHolds[0] = 0 或 mThresHolds[1] = 0时，结果为0，修改为：
...
2026-5-12 260512-201101001
==================
hp22mm.c中，恢复2025-11-4 - 11-13期间注释掉的代码段，记录ids的墨水使用情况，否则ids的用墨不被记录

2026-5-11 260511-201001001
2026-5-11 260511-200901001
==================
108mm不在apk内做镜像，下发给fpga做

2026-5-11 260511-200900001
==================
模式5，当网络断开的时候停止打印

2026-5-9 260509-200801001
==================
1. 取消260429-199501001版本做的偏移和重叠的功能。具体内容为：
   (1) 取消Jni中为此增加的变量定义
       //static jint ShiftBitsOdd=0;
       //static jint ShiftBitsEven=0;
       //static jint OverlapBits=0;
       以及对其的使用
   (2) 在BinFromBitmap类中，取消对于108MM做的特殊插入空隙的处理，插孔处理改为在FPGA中进行
   (3) C83和C84的追加暂时保留

2026-5-8 260508-200700001
2026-5-8 260508-200600001
2026-5-8 260508-200500001
2026-5-8 260508-200400001
==================
模式5的串口有PopupWindow改为RelightabeDiaog，用来支持屏幕变暗后，点击回亮。和配套的一些修改

2026-5-8 260508-200300001
==================
增加主页面网络状态变化时及时修改ip地址显示，在模式5的时候，显示Toast提示窗

2026-5-7 260507-200200001
2026-5-7 260507-200201001
==================
1. 取消200100001版本修改1，恢复原显示逻辑，将实时检测功能移到模式5的处理当中。当网络由连接转入断开时，显示红色提示框，并且报警，每三秒重复一次。用户点按OK键确认后，关闭提示框，停止报警
2. 修改变量高度计算逻辑
   (1) 修改MessageObject类中的getPixels函数返回值，由int型修改为float型，以避免产生较大的误差（变量高度本来就是以float保存的，因此保留float型才是正确的）
   (2) 修改getDisplayFs函数中高度的判断条件，以前加减0.3的方法在数值较小的时候误差比较大，改为根据靠哪个近取那个值

2026-5-6 260506-200100001
2026-5-6 260506-200101001
==================
1. 修改网络连接状态监视方法。
   (1) 全时实时监测
   (2) 当连接状态为连接时，黑色显示ip地址（与当前显示方法一致）
   (3) 当连接状态由连接变为断开后，每3秒报警一次，并且显示红色Disconnected字样。5次后停止报警，显示空
   * 开机即无连接时不报警，显示空
2. 模式5，当开机时不在从文件中读取当前打印状态数据，新数据需要用户手动下载

2026-4-30 260430-200001001
==================
修改260429-199501001版本的修改遗漏

2026-4-30 260430-199901001
2026-4-30 260430-199801001
2026-4-30 260430-199701001
2026-4-30 260430-199601001
==================
临时在108MM的信息后部追加72列的空格，其他的参数不变

2026-4-29 260429-199501001
==================
暂时启用了C83和C84两个参数。
C83设置偏移量，即原来是
[0,508) 是内容，[508,544)是插入的空挡，
C83=n的话，转化为：
[0,n)是空挡， [n,508+n) 是内容，[508+n,544)是插入的空挡，
【注意1：C83小于20时，设置1，3，5头偏移；大于20时，C83-20的值设置给2，4头】
【注意2：现在没有检查边界，需要合理设置】

C84为做了偏移后的508向后重叠的次数，即:
C84=n
[0-508)为内容，[508,508+n)为508的值，[508+n,544)空白
【注意：同样没有做边界设置，需要合理设置值】

2026-4-29 260429-199400001
2026-4-29 260429-199401001
==================
增加4L, 4R, 4BL, 4BR字体

2026-4-28 260428-199300001
2026-4-28 260428-199301001
==================
1. 修改libNativeGraphicJni中二值化的阈值，0xFF888888 -> 0xFFF0F0F0
2. 修改198900001版本修改1中的bug

2026-4-27 260427-199200001
2026-4-27 260427-199201001
==================
1. activity_main及activity_main2当中版本号显示字体由22sp修改为18sp
2. 取消260427-198900001版本修改3，恢复setAntiAlias(true)，因为false的情况下，5x5，7x6和7x6B的数字9会成为大白

2026-4-27 260427-199101001
==================
508复制给509

2026-4-27 260427-199100001
==================
取消260427-199000001的修改，缩小字号22so -> 18sp

2026-4-27 260427-199000001
==================
取消版本号的时间部分，否则太长

2026-4-27 260427-198900001
2026-4-27 260427-198901001
==================
1. 修改tlk文件的变量的x,xend,y,yend的定义。由于原来的位置2，3，4，5保存的是整型，必须保留，否则直接修改为浮点数的话，旧版apk读取新数据会在这里失败，返回0。因此，解决方法是：
   2，3，4，5位置继续保存整形，以保证与旧版的apk的兼容性。在最后不追加新的x,xend,y,yend的浮点数值，对应位置为22，23，24，25。
   在保存tlk文件时，分别写入整形和浮点数值，读取时，如果包含22，23，24，25项，则读取浮点数，如果不包括，则读取2，3，4，5位置的整形数
   修改日期统一为2026-4-25
2. 修改libNativeGraphicJni.so中的二值化算法，由原来的判断是否为0xFF000000，修改为判断是否小于0xFF888888，否则被灰度化的点位会被误判为白点
3. 将Paint的setAntiAlias(true)全部改为setAntiAlias(false))，取消看锯齿，否则会出现灰色内容。这样2的修改已经不需要，不过更加稳妥

2026-4-23 260423-198801001
2026-4-23 260423-198701001
==================
试验198601001版功能，非正式版

2026-4-23 260423-198601001
==================
特别处理108mm的倒置处理。由于在每个头的最后部添加了36点的空，因此倒置时这个空不能翻到最前面，而是要保留在原位

2026-4-20 260420-198500001
2026-4-20 260420-198501001
==================
将32DN类型打印头的适用于0-8之间的处理，扩充到0-10

2026-4-20 260420-198401001
==================
1. 更新7，12，14字体
2. 大幅修改bin数据生成逻辑，具体内容如下：    *代码中修改部分以2026-4-14为修改日期标签
(1) 问题的定义
    当生成bin数据的时候，生成速度非常慢，尤其是108mm打印头的情况下，生成一个YY-MM-DD格式的日期数据，需要耗时将近1分钟。

(2) 问题的分析
    生成bin数据需要四个步骤：
    S1: 生成bitmap图
    S2: getPixels获取bitmap的字节流
    S3: 添加打印头之间的空隙（1寸头系列，108mm头需要这个步骤增加两个相邻头之间的缝隙，这一部分没有实际的打印数据，半寸头的空隙在生成时完成不在这里处理）
    S4: 二值化（将bitmap的一个像素4个字节对应到bin数据中的一个位，原始字节流中的像素为先横向然后纵向，而bin数据为先纵向然后横向）

(3) 情况调查
    经过分析，主要耗时的地方是第4步的二值化处理，这里耗时的主要原因是pixel的保存顺序与bin数据的保存数据正好相差90度，因此在扫描所有数据的时候，总有一方需要在内存中进行大距离跳跃，数据量
    小的时候弊端不很明显，但是当数据量打的时候，会导致内存的频繁调度，从而导致性能的下降。那么推测如果能够在生成bitmap的时候，就将pixel的排列顺序与bin的位排列顺序同向的话，就应该避免掉这部分
    想能损耗。

(4) 试验
    在S1步骤生成bitmap的时候，从左向右排列的正视内容做如下变换生成：先以原点为中心顺时针旋转90度，然后再以竖轴为中心做镜像变换，就可以生成同向的位图。具体操作是：
	canvas.rotate(90, 0, 0);
	canvas.scale(1, -1, 0, 0);
    然后再绘制具体内容，就可以得到预期的位图。
    经过试验，S1步骤并不因为执行上述变化而额外增加开销，S2步骤不发生变化，但是二值化的处理会因为同向后处理性能大大提升，并且，同向处理时，可以在处理过程中插入空隙，这样S3步骤可以省略，
    测试结果如下（四组）：
	保存1.bin
	=======
	getPixels: 1034		getPixels: 1004
	shiftImage: 368		shiftImage: 1
	Binarize: 9092		Binarize: 544
	保存vbin
	=======
	getPixels: 1346		getPixels: 1394
	shiftImage: 483		shiftImage: 0
	Binarize: 13094		Binarize: 829
	保存1.bin
	=======
	getPixels: 891		getPixels: 893
	shiftImage: 409		shiftImage: 0
	Binarize: 9483		Binarize: 541
	保存vbin
	=======
	getPixels: 1388		getPixels: 1360
	shiftImage: 578		shiftImage: 0
	Binarize: 11755		Binarize: 854

(5) 代码修改原则
     在生成编辑图和预览图（给人看的）时，需要按着正常的方向生成，但是在生成bin数据（给打印头使用的数据）时，生成旋转镜像位图

(6) 代码修改内容
    (6-1) 对于NativeGraphicJni的修改 (NativeGraphicJni.so 1.0.7)
        (6-1-1) inarize函数，实现同向的pixel->bit的二值化，并且进行必要的空隙插入
        (6-1-2) ShiftImage函数，取消实际的移动功能，仅保存插入空隙的相应参数，供二值化时插入空隙使用
    (6-2) 对于MessageTask的修改，主要目标是保存信息时生成1.bin及vbin时的处理
        (6-2-1) 对于saveBinNoScale函数的修改
            由原来将不同变量横向排列，修改为纵向排列。变量本身的旋转镜像在变量内部的相应函数完成，此处仅修改不同变量的排列顺序
        (6-2-2) 对于BinCreator类的saveBin函数的修改
            由于该函数内会根据列高计算列数，而列高在经过旋转后，变为了列宽
        (6-2-3) 各个变量的处理方法
            (6-2-3-1) BarcodeObject
                通过makeBinBitmap绘制初始条码（如果是静态条码就是最后使用的条码，如果是动态条码该条码将会在打印过程中被替换）。不需要生成vbin
                在进入makeBinBitmap后，设置生成为bin使用的位图标识（mIsForBin）为true，然后分别生成：
                (1) drawQR: 生成QR二维码
                (2) drawDataMatrix: 生成DM二维码
                    根据mIsForBin标识，如果不为真，则生成一个正常的宽(w)，高(h)的位图，并且生成二维码后，填充进去；如果为真，则生成一个宽高互换的位图，生成二维码后，旋转90度后左右镜像处理后填充进位图。
                    旋转镜像处理通过BitMatrix->Pixels转换时完成。
                (3) drawOkapiQR: 生成GS1格式QR二维码
                (4) drawGS1Datamatrix: 生成GS1格式DM二维码
                    根据mIsForBin标识，如果不为真，则生成一个正常的宽(w)，高(h)的位图，并且生成二维码后，填充进去；如果为真，则生成一个宽高互换的位图，生成二维码后，旋转90度后左右镜像处理后填充进位图。
                    旋转镜像处理通过drawBitmap前设置Canvas的Rotate和Scale来实现。
                (5) draw: 生成一维条码，包括
                    - EAN8码
                    - EAN13码
                    根据mIsForBin标识，如果不为真，则生成一个正常的宽(w)，高(h)的位图，并且生成二维码后，填充进去；如果为真，则生成一个宽高互换的位图，生成二维码后，旋转90度后左右镜像处理后填充进位图。
                    旋转镜像处理通过BitMatrix->Pixels转换时完成。然后，将已经旋转镜像过的底图贴上后，旋转镜像写入文字信息
                    - GS1格式128码
                    - 其他条码
                    先生成一个正常宽高的地图，然后将底图及文字信息旋转镜像贴入画布
            (6-2-3-2) RealtimeObject
                (1) 修改getBgBitmap函数
                    该函数是生成背景图(1.bin)的函数，在此函数中首先生成一个旋转90度的画布，然后将子变量生成的旋转镜像后的图像按纵向贴到相应位置（原来是横向）
                (2) 修改BaseObject的makeVarBin
                    该函数生成vbin。在此函数中首先生成一个旋转90度的画布，然后在画布上旋转90度后镜像绘制内容。
                    - 首先，生成一个旋转90度的单数字画布，和一个同样旋转90度的包含0-9个数字的大画布
                    - 然后，分别生成旋转镜像后的0-9的数字图片，并且纵向贴到大画布中
            (6-2-3-3) HyperTextObject
                (1) 修改getBgBitmap函数
                    该函数是生成背景图(1.bin)的函数，在此函数中首先生成一个旋转90度的画布，然后将子变量生成的旋转镜像后的图像按纵向贴到相应位置（原来是横向）
                (2) 修改HyperTextObject的makeVarBin
                    生成每个需要vbin的变量的vbin，使用BaseObject的makeVarBin
            (6-2-3-4) GraphicObject
                    1.bin的处理。GraphicObject类的makeBinBitmap函数不做修改，因此得到的是一个正面图。在MessageTask的saveBinNoScale函数中，进行旋转镜像绘制到画布上。没有vbin
            (6-2-3-5) 其他变量
                (1) 1.bin，通过BaseObject的makeBinBitmap函数获取旋转镜像后的图片，按纵向贴到画布上
                (2) vbin，通过BaseObject的makeVarBin函数实现0-9的旋转镜像图片，二值化后保存

    (6-3) 对于DataTask的修改，主要目标是打印过程中生成动态内容的bin的处理
        具体到每个变量的处理方法为：
        (6-3-1) BarcodeObject
            通过getPrintBuffer获取打印缓冲区的位图，位图生成时即完成旋转镜像，然后通过BinInfo转化为bin数据。
            先生成一个旋转90度的图框作为容器（注意：对应于图片高度的参数为顶天立地的通高），然后与makeBinBitmap类似的方法，设置生成为bin使用的位图标识（mIsForBin）为true，然后分别生成相应的位图。由于此时生成的位图已经做过旋转镜像，
            因此，只要按着变量的纵向（由于做了旋转，因此实际为横向）的起始位置，贴到容器中
        (6-3-2) DynamicText
            通过getPrintBuffer获取打印缓冲区的位图，位图生成时即完成旋转镜像，然后通过BinInfo转化为bin数据。
        (6-3-3) HyperTextObject
            对于其中包含的DynamicText，通过getPrintBuffer获取打印缓冲区的位图，位图生成时即完成旋转镜像，然后通过BinInfo转化为bin数据。
        (6-3-4) 其他变量
            其他变量为使用vbin填充的变量，不需要特殊处理

2026-4-9 260409-198301001
==================
提高数据量大的信息（如108mm类型打印头的信息）的保存及打印时的处理速度和性能
1. 保存信息
   在没有优化前，保存一个Realtime类型的信息（YY-MM-DD）需要60s的时间，做了如下优化：
   1.1 当为符合变量的时候，仅生成一个自变量的vbin，其他子变量的vbin从最初的vbin复制，这样可以节省生成vbin的时间
   1.2 由于数据量大的时候，在jni的处理中需要跳行/列访问数据，导致内容频繁调度，浪费时间，修改为数据量大的时候分段进行二值化处理，具体以4M(4*1024*1024)为线，大于此大小时，分割为数块4M大小的位图进行分段二值化，然后拼接
   经过上述优化，保存的处理时间较少到18s左右
2. 生成打印缓冲区
   在没有优化前，上述数据生成打印缓冲区需要用时800ms。做了如下优化：
   2.1 只有第一次启动打印时才统计点数，统计点数的作业比较费时
   2.2 统计点数的算法做了优化，由原来的逐位位移统计修改为查表，可以缩短到原来的1/3的时间内完成
   2.3 改善BufferRebuilder类中即使没有任何边缘，也会在初始化时做char->byte的操作，完成时再做一次byte->char的操作，修改为只有需要变换时才进行此操作
   优化后第一次生成打印缓冲区用时180ms左右，第二次以后100ms左右

2026-4-3 260403-198201001
==================
取消借用HEAD_TYPE_1056向ko传递打印头类型为108mm的信息，改为根据R5的每列字节数计算108mm的合适的列数

2026-4-3 260403-198101001
2026-4-3 260403-198001001
2026-4-3 260403-197901001
==================
借用HEAD_TYPE_1056向ko传递打印头类型为108mm的信息，帮助ko计算合适的列数

2026-4-2 260402-197801001
2026-4-2 260402-197701001
==================
1. 修改Hp22mm.java类中
    HEAD_TYPE = HEAD_TYPE_1056
   以避免打印数据在ko中指定列数时按着循环头多指定一些列
2. 在Hp22mm.java类中，初始化时，如果Pairing失败，尝试对一下所有的SC卡，如果读失败则相应返回错误（-252，-251）
3. 修改主页面显示Hp22mm的错误的方法，由Toast显示改为AlertDialog显示
4. 初始化的失败信息也保存在错误文件中，会在错误显示窗中显示出来

2026-3-28 260328-197600001
2026-3-28 260328-197601001
==================
增加用户模式7，用以支持Custom.apk与Printer.apk之间通过Socket传递网络命令。在Printer.apk的主页面，点击扩展图标即启动Custom.apk，Custom.apk的具体用法另文说明

2026-3-27 260327-197500001
==================
与Custom.apk互启动，互通讯专用版

2026-3-24 260324-197401001
==================
1. 整理hp22mm.so中的log输出。访问错误的都以ERROR开头保存在ERR_STRING中，统计错误status_error的保存在ERR_STRING1中。
2. so返回给apk的错误信息为ERR_STRING1+ERR_STRING格式
3. apk中如果存在ERROR错误，则直接写入hp22mm_err_log文件，而ERR_STRING1的信息显示在屏幕中，只要有错误就会报警

2026-3-23 260323-197301001
==================
RunningState增加初始化为VALID，否则会出现开机加压后首次读取为INVALID而触发报警

2026-3-23 260323-197201001
==================
在ControlTabActivity类中，心跳处理部分，调用checkPress函数时恢复为只有BIGDOT机型时才调用，否则可能会与22mm的串口切换产生冲突

2026-3-20 260320-197101001
==================
...
(4) 当发生(2)或者(3)的错误时，显示在画面底部的错误栏的最前面

2026-3-20 260320-197000001
==================
方大特钢定制功能，当达到额定次数后，下发空白内容（以形式上维持打印次数限制，只是达到额定次数后，如果停止下发数据，则会打出来128个0x55，即竖线，当前方法可以避免竖线）

2026-3-19 260319-196901001
2026-3-19 260319-196900001
==================
再次完善checkPress的处理逻辑，如下：
	逻辑是：
	PE4  阀 1=on ， PE5 泵 1=on， PG9 液位   缺墨=0 。
	实现动作，  循环，   加墨， 底膜报警
(1).  循环
	a.当泵（P80、循环间隔）<=60s时，循环不工作, 避免循环太频繁
	b. P80 > 60 s ， 每经过P80 时间， 压力高于p78，PE5 拉高 1s.   PE4 拉低。

(2).   加墨：
	a. 每1秒检测一次，
	如果从ADS1115读取的值，经过计算
			press = ADS1115 / 32767 * (4.0 * 66.7 + 12) - 6.67
	b.   得到press，小于P78的设定值时，并且PG9为高时，拉高PE4与PE5   1s   (都开 加墨 )

(3). 报警 ：  PG9为低时报警

2026-3-19 260319-196801001
2026-3-19 260319-196800001
==================
再次完善checkPress的处理逻辑，如下：
	逻辑是：
	PE4  阀 1=on ， PE5 泵 1=on， PG9 液位   缺墨=0 。
...
(1).  循环
	a.当泵（P80、循环间隔）<=60s时，循环不工作, 避免循环太频繁
	b. P80 > 60 s ， 每经过P80 时间， 压力高于p78，PE5 拉高 1s.   PE4 拉低。 如果压力低于p78，PE4   同时拉高 1s.
...
2026-3-19 260319-196700001
==================
方大特钢定制功能增加每个数据限制打印次数

2026-3-19 260319-196601001
2026-3-19 260319-196600001
==================
再次完善checkPress的处理逻辑，如下：
	逻辑是：
...
2026-3-18 260318-196500001
2026-3-18 260318-196400001
==================
重新整理260314-196000001版本对RTC时间的修改逻辑。
开机后，每隔5秒检查一次系统时间与RTC的时间当发现下述情况时，
1. OSF为高位 或者
2. WH为高位 或者
3. 当前日期为2000-1-1 时
在画面底部显示OSF(1) WH(1) 2000-1-1等错误信息，
当第一次检查（即开机后的检查）时发生错误的情况下，显示有“请检查时间”字样的提示对话窗，如果用户点击OK则进入时间设置对话窗，重新设置时间，如果用户通过设置时间对话窗将时间修正后，WH，OSF高位标识将被清除；
如果用户点击Cancel，则直接清除WH，OSF高位标识
4. 当系统时间与RTC时间相差5秒以上时
直接将系统时间写入RTC，并且清楚WH，OSF高位标识

2026-3-17 260317-196301001
==================
1. 修改BaseObjcet当中resizeByHeight函数，明确重新设置一下字体
2. 重新整理该部分逻辑ControlTabActivity类的checkPress函数），处理内容以本次说明为准
	逻辑是：
	PE4  泵 1=on ， PE5 阀 1=on， PG9 液位   缺墨=0 。
	实现动作，  循环，   加墨， 底膜报警
(1).  循环
...
(2).   加墨：
	...
	如果从ADS1115读取的值，经过计算
			press = ADS1115 / 32767 * (4.0 * 66.7 + 12) - 6.67
	b.   得到press，小于P78的设定值时，并且从PG9=1，拉高PE4和PE5   1s   (都开 加墨 )

(3). 报警 ：  PG9为低时报警

2026-3-17 260317-196201001
==================
完善108mm的倒置处理，在1头倒置设置为0时设置revert=0，而不是直接跳过

2026-3-16 260316-196101001
2026-3-16 260316-196001001
==================
108mm的实际高修改为508

2026-3-14 260314-196000001
2026-3-14 260314-195900001
==================
重新整理260312-195300001以后版本对RTC时间的修改逻辑。
开机后，每隔5秒检查一次系统时间与RTC的时间当发现下述情况时，
1. OSF为高位 或者
2. WH为高位 或者
3. 当前日期为2000-1-1 时
在画面底部显示OSF(1) WH(1) 2000-1-1等错误信息，并且黄色Toast显示“请检查时间”字样提示，
如果用户通过设置时间对话窗将时间修正后，WH，OSF高位标识将被清除
4. 当系统时间与RTC时间相差5秒以上时
直接将系统时间写入RTC，并且清楚WH，OSF高位标识

2026-3-14 260314-195800001
==================
向设置RTC设置时间是，清楚OSF表示的高位

2026-3-14 260314-195700001
==================
修改RTC控制的逻辑，当出现OSF,WH高时在画面底部显示OSF(1) WH(1)字样
...
==================
1. 更新乌克兰文资源文件
2. 增加方大特钢的定制功能，使用扫描协议12

2026-3-9 260309-195001001
==================
增加升级apk后执行一次sync，以尝试避免F2.txt的内容没有被更新

2026-3-9 260309-195000001
==================
增加乌克兰语

2026-3-9 260309-194900001
2026-3-9 260309-194901001
==================
借用心跳线程，每隔5秒钟来比对RTC当中保存的时间和系统时间的差异，如果相差超过5秒就报警

2026-3-4 260304-194800001
2026-3-4 260304-194801001
==================
取消DataTransferThread中根据是否使用了FIFO和SCANNER3协议分别处理打印完成的处理逻辑，这个逻辑是没有从img获取到实际打印计数之前的近似逻辑，img可以返回打印计数后这个逻辑应该取消，属于修改疏漏

2026-3-3 260303-194701001
==================
放开enable和disablewarming的管理；并且放开get_temperature_override功能
（正式版本，读两个头的状态，读temperature_override）

2026-3-3 260303-194601001
==================
在monitorThread中主动disable_warming

2026-3-3 260303-194501001
==================
再回到平铺的流程，在循环中好像确实不行

2026-3-3 260303-194401001
==================
1. 恢复两个头在循环中操作，但是保持 pd_get_temperature 在每个循环处理的最后处理，以免影响后续处理
2. 暂时取消加热和关闭加热

2026-3-3 260303-194301001
==================
将 pd_get_temperature 挪到交互操作的后面，避免这个函数带来的影响

2026-3-3 260303-194201001
==================
修改波兰语资源文件

2026-3-3 260303-194101001
==================
暂时取消第二轮的 pd_get_temperature，看看后续的pd与ids的交互是否还受影响

2026-3-2 260302-194001001-P2
==================
将monitorThread函数中受pd_get_temperature函数影响的循环执行方式修改为平铺两次代码的方式，以避免上述影响循环变量的问题

2026-3-2 260302-193901001
==================
hp22mm库，monitorThread函数中，取消 pd_get_temperature_override和pd_get_recirc_override的调用，开放读取温度的pd_get_temperature函数，但是只读头1的温度，因为读头2的温度会影响循环变量，原因不明

2026-3-2 260302-193801001
==================
暂时恢复528测试一下打印效果

2026-2-28 260228-193701001
==================
修改hp108mm的bug。高度由528修改为516
...
==================
1. 260207-192700001版本3修改项，宝桥定制功能优化
2. 计数器预览页面显示字体大小放大

2026-2-7 260207-192800001
==================
260207-192700001版本3修改项，宝桥定制功能优化

2026-2-7 260207-192700001
2026-2-7 260207-192701001
==================
1. 修改了22mm的情况下，DataTransferThread当中阈值的计算方法。修改方法为：
    当mThresHolds[0]或者mThresHolds[1]有一个是0的时候，不在采用P1*P2/(P1+P2)，否则结果是0，改为：
        当P1=0并且P2=0时，阈值给与一个足够大的数，即几乎无论怎么打印都不减少墨水
        当P1=0或者P2=0时，等于0的值不参与计算，B的值取不为0的那个值
        其余情况正常计算
2. 修改了so升级的方法。由原来的升级了apk后从asset目录复制到指定位置，修改为升级的当时，从apk复制，此时需要将apk视为一个压缩文件，从压缩文件中提取文件复制
3. 增加宝桥定制功能，通过用户模式6启动。

2026-2-5 260205-192601001
==================
在TestHp22mm中追加一个高频率测试打印头状态的功能（每0.1秒执行一次读取信息，正常运行时每两秒一次）
操作方法：打开hp22mm的测试页，找到最后一项“Read Print Head Error every 100ms”，点击一次开始测试，下面会显示
0:15
1:0
...
样式的内容。冒号前是错误号，冒号后是次数
再次点击“Read Print Head Error every 100ms”停止测试，恢复到正常运行状态

2026-2-5 260205-192501001
==================
修改hp22mm.c中手机ERR_STRING1的方法，取消只有错误是才收集，改为任何时候都收集，否则apk一旦发生错误后就会一直报警

2026-2-5 260205-192401001
2026-2-5 260205-192301001
==================
22mm长清洗时，R15= 15，000.   R16=60，0000.，  R18=2 ,  关加热

2026-2-4 260204-192201001
==================
放开DataTransferThread类中clean函数中，开始清洗前，22mm的情况下禁止加热
				if(PlatformInfo.getImgUniqueCode().startsWith("22MM")) {
//					remainTime *= 2.17;
					Hp22mm.EnableWarming(0);
				}

2026-2-3 260203-192101001
2026-2-3 260203-192001001
==================
260203-191901001的修改1的bug修正

2026-2-3 260203-191901001
==================
1. 修改hp22mm守护线程时的错误显示和报警方法。具体内容如下：
   假如数次获取错误码的序列为 0，0，0，0，14，0，0，14，15，15，0，0，0，则显示出来的错误信息为 14(2),15(2),并且在收到非0时报警，收到0时不报警
2. hp22mm.so中增加一个CheckPenStatusAndRepowerOnIfNeed函数，用来在每次打印后获取打印头状态，并且在必要时启动重新上电（so版本1.0.183）

2026-2-3 260203-191801001
2026-2-3 260203-191701001
==================
1. hp22mm,清洗时
R15=15,000
R16=30,000,000。
2. libhp22mm.so，修改为可以重新上电的正式版（so版本1.0.182）

2026-2-2 260202-191601001
==================
hp22mm.c中，pd_get_print_head_status发生错误时，同时返回错误号（so版本1.0.181）

2026-2-2 260202-191501001
==================
调整屏保功能，与A20动作一致，开机暗显，有触屏操作点亮，3分钟无操作恢复暗显

2026-1-31 260131-191401001
==================
临时增加一个ERR_STRING1，用来保存monitorThread函数中获取的 pd_get_print_head_status的错误信息，否则ERR_STRING会在下一条函数成功后被清空，导致报错信息不会被送回（so版本1.0.180）

2026-1-31 260131-191301001
==================
在monitorThread函数中，读取 pd_get_print_head_status后，即使函数返回的是PD_OK，也要检查一下print_head_status.print_head_state和print_head_status.print_head_error，并且在必要时返回错误信息（so版本1.0.179）

2026-1-31 260131-191201001
==================
将260129-190601001版本(so版本1.0.177)放开的重新上电功能再次暂时关闭(so版本1.0.178)

2026-1-30 260130-191101001
2026-1-30 260130-191100001
2026-1-30 260130-191001001
2026-1-30 260130-191000001
2026-1-30 260130-190901001
2026-1-30 260130-190900001
2026-1-29 260129-190801001
2026-1-29 260129-190800001
==================
1. 修改Hp22mm.java类中R18, R23和R24的计算公式。
   R23 = 1
   R24 = 26*(100/C59) C59<10时按100计算
   R18 = ((C10*2*25.4)/(C9*3.14))/C3A*32*(C59/100), C59=0时，按100算，C59=1-8，R18=C59
2. C59增加显示单位：%

2026-1-29 260129-190701001
2026-1-29 260129-190700001
==================
修改模式5的搜索时bug

2026-1-29 260129-190601001
2026-1-29 260129-190600001
==================
1. hp22mm.c 在monitorThread函数中，恢复2025-12-3取消的失败时重新尝试上电的功能，so版本号1.0.177
2. Hp22mm.java类中，取消R23和R24临时取C90和C91值的设置，恢复为原值R23=0；R24=25.但是该参数已经在底层写死，设置没有实际意义
3. Hp22mmSCManager.java类中，修改错误信息的取得和显示逻辑，当libhp22mm.so第一次返回错误，不在取得新的错误字串，也一直显示该错误
4. 模式5修改为，当开始搜索，启动从服务器更新数据和删除时，自动停止打印。开始打印后，打印按键显示灰色变为不可点按，停止打印后恢复

2026-1-28 260128-190500001
2026-1-28 260128-190400001
==================
模式5的搜索后，重新下发新数据

2026-1-28 260128-190301001
==================
hp22mm，临时启用C90和C91作为R23和R24的值

2026-1-27 260127-190201001
2026-1-27 260127-190200001
==================
模式5的按键“内容”修改为“刷新”

2026-1-27 260127-190101001
2026-1-27 260127-190100001
==================
修改189800001中从服务器导入数据时显示的bug

2026-1-27 260127-190000001
==================
在189900001基础上，完善删除目录中含有目录的情况（重写deleteTlkDir函数，增加递归调用）

2026-1-27 260127-189900001
==================
取消MessageBrowserDialog中的onItemClick函数中对于是否存在1.tlk的判断，是的没有1.tlk文件的目录也可选。好处是用户错误导入的无1.tlk的目录也可以删除，但是打开会无动作

2026-1-27 260127-189800001
==================
在模式5中，修改检索方法。
原方法：检索到的第一个命中条目显示到清单可视范围的最前面
现方法：只显示命中的项目，关键字用红色显示。未命中项目不显示
删除操作时，取消检索条件，所有项目均删除，但保留服务器删除失败的项目

2026-1-23 260123-189701001
2026-1-23 260123-189700001
==================
修改so的升级逻辑：
当A20平台时，在PrinterApplication的onCreate函数中升级，由于A20升级so后即可有效，因此直接载入so库就可以
当A133平台时，不在PrinterApplication的onCreate函数中升级（其实在这里升级的话，升级的操作是没有问题的）。由于A133的so不能直接复制到目标位置，需要先复制到一个中间位置，然后重启后在由系统脚本复制到目标位置，
因此，升级so后需要重启才会有效，这样在WelcomeActivity中升级的话，方便判断是否又升级操作，如果有则提示重启，这个是放在WelcomeActivity类中，而不是PrinterApplication类中的原因。
同时，由于原来的WelcomeActivity类中，在onCreate刚开始就执行了Configs.initConfigs(mContext)，而在这个函数执行过程中，会调用到SmartCard的so，因此这个调用需要修改到升级完成后不再有升级的地方调用，否则，在A133平台，会出现apk升级
后，准备升级so时，apk的so接口类（新apk中）与so本身（还是旧的so）的api接口如果不一致，会出现崩溃的问题

2026-1-22 260122-189601001
2026-1-22 260122-189600001
==================
1. 修改模式5中显示清单中的条目支持横向滚动（单纯支持横向滚动会导致点击和长按事件被屏蔽，对此做了优化处理，处理方法是给每个控件都添加点击和长按事件响应，并且统一完成相同的原有点击和长按操作）。点击为选择项目，长按是显示详细信息
2. 临时模式5修改删除条目的url为InkPrintTest，待确定后恢复为InkPrint
3. 修改so的载入时机，由PrinterApplication类转移到WelcomeActivity，目的是只有升级成功才load库，否则库的api接口有修改时，A133会出现先load（旧的so）后升级（新的so）的问题，导致apk的api与so的api不匹配而崩溃的问题

2026-1-21 260121-189500001
2026-1-21 260121-189501001
==================
22mm。根据22mm_SS_SW_01_02版本中的修改内容，升级print_head_driver.c和extension.h中的相应内容

2026-1-20 260120-189400001
==================
对于快捷模式(参数P71)MODE5做一下修改：
1. 修改内容显示字体大小，由24sp扩大到36sp（扩大50%），并且增加一倍行间距
2. 搜索字段扩大到所有字段
3. 增加一个删除按键，点按按键后删除所有记录，每删除一条向服务器回复一条，内容为在打印完成后的内容基础上，增加一个Cmd:N的字段
...
2026-1-13 260113-189101001
==================
增加108型22mm打印头的FPGA的升级方法（包括TestHp22mm中的升级调用连接和hp22mm的so库）

2026-1-12 260112-189100001
==================
注释掉RfidScheduler中的基础mManager.switchRfid函数调用，感觉好像没有意义，并且可能导致乱切换锁，改在实际需要访问的时候切换（此时，可能也不会再发生冲突了）

2026-1-12 260112-189000001
==================
修改GraphicObject当中setHeight时重新计算宽度的算法，原来保存图片时会出现图形拉伸的问题，原因是在BaseObject中保存字体时，会根据内容重新评估了宽度，而GraphicObject在mContent中保存的是图片的路径信息，所以宽度就被按着字符串计算宽度了。
修改为：根据新高度与图片高度的比值调整图片的宽度，这样会得到按着高度比例调整的宽度

2026-1-12 260112-188900001
==================
1. 取消188600001-188800001对ExtGpio.rfidSwitch函数后等待时间的修改，恢复原值。因为造成发生锁值异常变化的原因是：
(1) 由于有为初始化成功的锁，因此在循环启动初始化，(2) 点击打印后启动检查UID的处理，(3) 打印过程中实时写入锁值变化
上述三种操作均可能切换当前锁，而影响到其他操作访问到错误的锁。因此采取了一下措施：
(1) 检查UID和初始化处理做互斥处理
(2) 当开始打印后，停止循环启动初始化（以后也不在尝试初始化）
这样就避免了三者的互相干扰
2. 增加扫描协议11

2026-1-9 260109-188800001
==================
188700001中修改为350ms的等待时间都修改为400ms

2026-1-8 260108-188700001
==================
188600001中修改为300ms的等待时间都修改为350ms

2026-1-7 260107-188600001
==================
将所有调用ExtGpio.rfidSwitch函数的后面都修改为300ms（原来有200，100和50ms的），以避免切换锁的时候为切换成功的情况发生

2026-1-7 260107-188500001
2026-1-6 260106-188400001
==================
开机后，读锁值完成前打印键修改为不可点击

2026-1-6 260106-188300001
==================
在ControlTabActivity类中，
				case SmartCardManager.MSG_SMARTCARD_CHECK_SUCCESS:
				case RFIDManager.MSG_RFID_CHECK_SUCCESS:
				case MESSAGE_PRINT_START:
相应段，当为鸡蛋机的时候不检查checkRfid

2026-1-6 260106-188200001
2026-1-6 260106-188100001
==================
1. 188000001版本基础上，当if(null == mDevice.mSN)时，先调度到下一个锁，然后返回
2. 基于188000001，在RFIDManager类中处理MSG_RFID_CHECK_SWITCH_DEVICE消息代码段，增加对于鸡蛋机的特殊处理，只要有效打印头数大于3，就返回成功，使得能够继续打印

2026-1-6 260106-188000001
==================
在RFIDManager类中的相应处理位置上，增加
						if(null == mDevice.mSN) return;
以避免在允许读锁失败情况下打印时，这里会出现异常

2026-1-5 260105-187900001
==================
修改251125-185200001中的内容，从一个锁值有问题放行，改为三个锁值有问题放行

2026-1-4 260104-187800001
2026-1-4 260104-187801001
==================
1. 修改32x6和32x7类型打印头，在选择大小为7x6, 10x8, 12x9, 14x10, 16x12, 19x13, 21x14时，个别选择值会发生错位的问题，主要是由于分母点数比较多（192，224）导致相邻两个大小的计算判定范围会落到同一个区间
2. 修改当NOZZLE_INDEX_64DOTONE, NOZZLE_INDEX_64_DOT, NOZZLE_INDEX_64SLANT, NOZZLE_INDEX_64SN时，下发给FPGA的打印头数为2（原来下发的为1）.以及当NOZZLE_INDEX_96DN时下发3（原来为1）

2025-12-30 251230-187701001
==================
修改A133的电池存量获取方法，皆有ExtGpio类（和底层的ko）实现。需要ext_gpio.ko版本7003以后版本的支持

2025-12-29 251229-187600001
2025-12-26 251226-187601001
==================
1. 在PrinterNozzle类中，32x3 - 32x7打印头类型的定义头数修改为1，大字机头数都按1算，目的是解一个锁，但是下发FPGA的时候需要按实际指定头数
2. 在FpgaGpioOperation类中，下发头数当打印头类型为32x2-32x7时，按实际头数下发头数

2025-12-22 251222-187501001
==================
修改TestMain中的Save Test001的显示方法，之前使用的Toast显示方法不能及时更新进度

2025-12-19 251219-187401001
==================
修改251215-187101001的bug

2025-12-16 251216-187301001
==================
增加12D字体

2025-12-16 251216-187201001
==================
（2025-12-15修改）
1. 修改font的升级方法，放置到U盘下的fonts目录中，使用USB->Printer进行升级，只支持开头是数字，并且没有扩展名的文件
2. 修改186901001版本中记录ControlTabActivity类的启动时间的方法，不使用System.currentTimeMillis，而是使用SystemClock.uptimeMillis()，因为前者与系统时钟有关，有时候系统启动时始终不正确
...
以避免mThresHolds[0] + mThresHolds[1]为0时产生NaN

2025-11-25 251125-185301001
2025-11-25 251125-185300001
==================
251121-184600001当中启用的第六个参数改为使用第七个

2025-11-25 251125-185200001
==================
在ControlTabActivity类的refreshInk函数中，在一个检测打印头状态的循环中，如果是鸡蛋机，则统计出现有问题头的数量，当有两个以上头有问题时报警禁止打印，只有一个头时红色显示，但不报警，也不禁止打印

2025-11-24 251124-185100001
2025-11-24 251124-185000001
==================
1. 251124-18490001中的清洗提示窗不消失，只是清洗完成后，提示完成关机
2. 清洗时间：10分钟 -> 20分钟

2025-11-24 251124-184900001
==================
1. 墨水类型不匹配时，不启动打印
2. 清洗10分钟后不允许打印及再次清洗

2025-11-24 251124-184800001
==================
【临时】apk的inkID=2

2025-11-24 251124-184700001
==================
1. 增加导出img中的fpga-sunxi-ko.log的功能。保存到U盘的根目录下，文件名为：fpga-sunxi-ko.log
2. 修改184600001版本中增加的墨水识别号的显示功能：
    当RFID中的inkID==1时，显示不可取消的提示窗，10分钟后停止打印，取消提示窗显示
    当RFID中的ID与apk的ID不同时，显示可取消的提示窗，并且报警

2025-11-21 251121-184600001
==================
...
2025-11-19 251119-184500001
2025-11-19 251119-184400001
==================
修改EditTabSmallActivity类中的保存处理，当保存信息不包含任何内容时，提示错误，并且退出保存

2025-11-17 251117-184301001
2025-11-17 251117-184201001
2025-11-17 251117-184101001
2025-11-17 251117-184000001
2025-11-17 251117-184001001
==================
修改剩余次数的计数方法，取消原来的=R*T+C的计算方法(R:锁值，T:阈值，C阈值内剩余次数)，改为开始打印时计算一次，以后每次打印减1

2025-11-14 251114-183900001
2025-11-14 251114-183901001
==================
1. 22mm跳跃式减记问题修改
2. 修改内容后反映在正在打印的任务中
3. 22mm缩小无效

2025-11-14 251114-183800001
==================
将DataTransferThread类中的PrintTask中的守护线程，睡眠时间从3ms修改为2ms

2025-11-13 251113-183700001
2025-11-13 251113-183701001
==================
对于251112-183500001版本的预览区长按打开编辑功能。
1. 缩小显示区大小到原来的80%
2. 对话窗全屏显示

2025-11-12 251112-183600001
==================
取消251111-183000001对睡眠位置的调整，恢复到原来的地方

2025-11-12 251112-183500001
2025-11-12 251112-183501001
==================
完善251110-182801001版本的预览区长按打开编辑功能。
1. 点击群组中信息选择窗的OK后，进入编辑页面，并且直接进入第一条变量的属性编辑窗
2. 完善显示功能，使得点击选择信息更加快速
3. 修改启动参数，由全路径修改为信息名，否则影响保存功能

2025-11-11 251111-183401001
==================
R24=25

2025-11-11 251111-183301001
==================
R24=28

2025-11-11 251111-183201001
==================
R24=C4

2025-11-11 251111-183101001
==================
R24=C3

2025-11-11 251111-183000001
==================
将PrintTask当中的守护线程循环的睡眠3ms从底部调整到顶部，目的是看看躲过PH14之后下发线程启动前与之争抢资源

2025-11-10 251110-182900001
==================
1. 增加两个ko的ioctl调用，一个是开始生成打印缓冲区标识，另外一个是结束打印缓冲区生成标识，用来在img的memlog中进行记忆，使得两套计时可以统一到一个平台进行统计

2025-11-10 251110-182801001
==================
增加一个在预览区长按后选择群组中信息的对话窗，然后直接进入编辑的功能。与UI_TYPE==UI_CUSTOMIZED0的长按直接编辑功能合并。当所选内容为群组时，进入群组选择信息对话窗，如果是信息，则直接进入编辑页面

2025-11-7 251107-182701001
==================
22mm长清洗时，减锁直接调用Hp22mmSCManager的downLocal函数，不在通过界面中转，看看卡不卡屏

2025-11-7 251107-182601001
==================
22mm长清洗时，每次需要减锁时，减锁10次，达到减10个锁的效果。中间插入睡眠，否则有卡住的现象

2025-11-7 251107-182501001
==================
R15=15,000

2025-11-7 251107-182401001
==================
R16=30,000,000。

2025-11-7 251107-182301001
==================
hp22mm.java类中R15=8,000; R16=12,000,000。
DataTransferThread类中的长清洗函数，22mm的暂时下发544*1024个char的数据

2025-11-7 251107-182201001
2025-11-7 251107-182101001
2025-11-7 251107-182000001
2025-11-7 251107-182001001
==================
增加22mm长清洗时减锁的功能，暂时用于测试和调试，还未确定

2025-11-6 251106-181901001
==================
DataTransferThread类的getInkThreshold函数中，将语句
bold = (int)bold
修改为
bold = (int)Math.max(1.0f, bold)
否则当选择150dpi的时候，bold会等于0， 导致0作为除数，而计算出来的Threshold值为Infinity，最后导致mcountdown的相应值为NaN

2025-11-6 251106-181801001
==================
继续找不显示剩余次数的问题

2025-11-6 251106-181701001[已验证无效]
==================
将DataTransferThread类中的两处
mThresHolds[i] == 0 改为 mThresHolds[i] == 0f
怀疑float类型的0f可能内部不等于0
还有，为mThresHolds赋初值
private float[] mThresHolds = new float[] {0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f}
目的是尝试当分辨率设置为150，450，750的时候不显示剩余次数的问题，根据log，此时
...
中的mcountdown[i]为NaN，可能是计算remainRatio时出现了0为除数，即mThresHolds[i]的0f与原式中的0不相等，导致除了mThresHolds[i]，这个值可能是没有赋值，或者是0

2025-11-5 251105-181601001
==================
修改R15的计算公式，当C2的值不是300的整数倍时，令其按小于他的300整数倍值中的最大值计算，150按300计算

2025-11-5 251105-181501001
==================
hp22mm.java类中，R18的计算公式修改，当C6=0时，追加C3按300算

2025-11-5 251105-181401001
==================
hp22mm.java类中，R18的计算公式修改，当C6=0时，按C9=64，C10=1200算

2025-11-5 251105-181301001
==================
1. hp22mm.java类中，R18的计算公式修改，当C6=0时，按C9=1200，C10=64算
2. ControlTabActivity类中修改小于5%的时候，只报警，不亮红灯；小于0%时即报警又亮红灯
3. RfidScheduler类中的readLevelValueA133函数修改忽略掉参数cardIdx，每次对所有的头的读数进行更新

2025-11-5 251105-181201001-NoOIB
==================
不写入OIB，不写锁值的长测试专用版本

2025-11-5 251105-181101001
==================
hp22mm.java类中R15=15,000; R16=25,000

2025-11-5 251105-181001001
==================
hp22mm.java类中R15=30,000; R16=50,000

2025-11-5 251105-180901001
==================
hp22mm.java类中R15=30,000; R16=150,000

2025-11-5 251105-180801001
==================
hp22mm.java类中R15=30,000; R16=350,000

2025-11-4 251104-180701001
==================
ConfigPath类中的getMountedUsb函数中，增加判断当前识别的U盘有效性的功能。当没有登记U盘时，重新查询一次，以避免开机只查一次，但U盘一直插着，系统识别U盘在后，导致识别不到U盘的问题

2025-11-4 251104-180700001
==================
1. 更新字体4和7
2. ExportLog2Usb后睡眠5s修改为20s
3. 增加PH15的读取

2025-11-4 251104-180600001
2025-11-4 251104-180500001
==================
在ImportDialog的Log->USB按键响应函数中，输出log前读取一下PH14的状态

2025-11-4 251104-180501001
==================
修改锁值显示的地方，不同行的空位显示GONE，同行的空位显示INVISIBLE

2025-11-4 251104-180401001
==================
1.
在SystemConfigFile类中，增加一个版本号CURRENT_VERSION，代表当前版本apk对应的参数保存文件system_config.xml所对应的版本。apk根据从xml文件中读取的版本号与apk中保存的版本号的对比，确定是否需要对保存的参数进行相应的调整。
如：选择打印头类型的参数，在251030-179501001版本之前，如果选择的打印头类型是16DOT，则保存xml文件时，保存的是其序号7，但是在251030-179501001版本之后，由于在其前面加入了8中打印头类型，如果新的apk读取了旧版本apk保存的xml文件
则，读取的值7就对应了25.4型打印头，导致错误。
本次增加主要是为了解决这个问题
* 在对参数做了需要调整的修改之后，版本号需要从1开始连续增加。调整的策略是从最早的版本开始逐步调整到最新的版本

2.
在ControlTabActivity类中的refreshInk函数中，调整报警蜂鸣的机制，好像墨盒为空后可能不报警

2025-11-4 251104-180301001
==================
修改251030-179501001版本中增加的25.4x5 - 25.4x8打印头类型得字号计算错误

2025-11-3 251103-180300001(取消)
==================
修改为在所有情况下，预览区长按后进入编辑页面
在DataTransferThread类中的getInkThreshold函数中，hp22mm的情况下，忽略调150， 450， 750等选项（计算时不考虑），只保留300，600，900，1200的选择

2025-11-1 251101-180201001
==================
hp22mm.java类中R16=700,000

2025-11-1 251101-180101001
==================
增加ContorlTabActivity页面支持8个头的锁值显示功能

2025-10-31 251031-180001001
==================
1. 修改ControlTabActivity类中refreshInk函数中的停止打印的条件，由0f修改为0.1f
2. 将长清洗的时间由2分钟修改为5分钟，并且统一22mm的时间（不在乘以2.17）
3. Hp22mm.java中长清洗的R16修改为90000*2.5=225000

2025-10-31 251031-179901001
==================
1. 修改251030-179501001版本扩展到8个头的功能，下发FPGA的打印头数，Paramter类中的S17的设置中，原来使用0xe7f作为Mask，只能设置S17[8:7]的四个头，修改为使用S17[10:7]可以指定16个头。
2. S17[10:7] = 0000 : 一个头
   S17[10:7] = 0001 : 两个头
   ...
   S17[10:7] = 1111 : 十六个头
3. 头数的计算方法
   S38未指定n带m时，头数 = 打印头类型中指定的头数
   S38指定n带m时，头数 = m

2025-10-30 251030-179801001
==================
RfidScheduler类中的init初始化时，由原来都是根据LEVELS的定义生成数组，修改为根据平台，如果是A20的，则生成LEVELS定义的个数的变量（4个），A133时生成16个变量。并且A20是通过SmartCard类的i2c驱动读取level数值，而A133通过Fpga的ko，通过spi访问读取数值，最多16个数值

2025-10-30 251030-179701001
2025-10-30 251030-179601001
==================
临时修改Hp22mmSCManager中
1. 无论初始化成功还是失败，均返回成功
2. getLocalInk和getLocalInkPercentage返回固定有效值
3. isValid始终返回true
测试一下看看能不能打印

2025-10-30 251030-179501001
==================
增加12.7的x5，x6，x7，x8头类型及25.4的x5，x6，x7，x8头类型，修改的地方比较多，根据2025-10-29日期为标签可以查到修改处

2025-10-29 251029-179401001
==================
Hp22mm的getSettings函数中，取消计算encFreq时，将config.mParam[0]最小值由110改为55

2025-10-29 251029-179301001
==================
在Hp22mm中增加HEAD_TYPE变量标识不同的打印头，
...
在DataTask生成打印缓冲区时，以及PrinterNozzle类中给出打印头的高度和纵向倍率时参照。并且下发至img，供img在计算列数时参照

2025-10-28 251028-179201001-NoOIB
==================
不写入OIB，不写锁值的长测试专用版本

2025-10-28 251028-179100001
2025-10-28 251028-179101001
==================
增加网络设置打印技术设置功能。使用当前设置计数器值的网络命令（Counter），使用Index=255，Value(在note2位置）=设置值，当值为0是等同于清零

2025-10-28 251028-179001001
==================
1. PI11Monitor类中读取Gpio管脚的时间间隔由500ms修改为200ms
2. 短清洗参数修改为：R15=200,000; R16=15,000,000
3. 短清洗持续时长改为：2.9s

2025-10-28 251028-178900001
2025-10-28 251028-178901001
==================
PI11Monitor类中读取Gpio管脚的时间间隔由500ms修改为250ms

2025-10-27 251027-178801001
==================
临时修改长清洗时下发参数中
R15=60，000   R16=90,000

2025-10-27 251027-178800001
2025-10-27 251027-178700001
==================
替换下列字体
1+，10，10B，12，14，15，19，21，24，4，4B，5X5，7，7B

2025-10-26 251026-178600001
==================
增加静态文本中添加字符间距的功能。
工作原理：在字符间插入若干个特殊字符（倒置的感叹号（"¡"）），每个特殊字符作为一个单位的间隔，由该字符的特制字体实现空格的效果
修改内容：
1. 在静态文本变量属性编辑窗中追加一个 字间距/Spacing 属性，输入数字，该数字即为单位空格的数量
2. 在设置静态文本的内容时，根据字间距属性，在每个字符间加入相同数量的特殊字符，之后的处理均使用该加入特殊字符的字符串作为内容
3. 在属性编辑窗中显示剔除特殊字符的内容
4. 在保存文件时保存剔除特殊字符的内容
5. 在tlk文件中，静态文本的第20项（即字体和内容之间的项目）作为字间距的保存项使用
6. 仅支持大字机，非大字机时忽略字间距属性

2025-10-24 251024-178501001
==================
临时修改长清洗时下发参数中
R15=60，000，   R16=4,000，000

2025-10-24 251024-178401001
==================
临时修改长清洗时下发参数中
R15=60，000，   R16=5,000，000

2025-10-24 251024-178301001
==================
临时修改长清洗时下发参数中
R15=60，000，   R16=1,100，000

2025-10-24 251024-178201001
==================
临时修改长清洗时下发参数中
R15=60，000，   R16=900，000

2025-10-24 251024-178101001
==================
临时修改长清洗时下发参数中
R15=60，000，   R16=800，000

2025-10-24 251024-178001001
==================
临时修改长清洗时下发参数中
R15=60，000，   R16=700，000

2025-10-24 251024-177901001
==================
临时修改长清洗时下发参数中
R15=100，000，   R16=1，000，000

2025-10-24 251024-177801001
==================
改 R15=900,000
R16= 15,000,000

2025-10-22 251022-177701001
==================
hp22mm, 采用新公式 R20= C4x12+c11， R21= C4x12+c12

2025-10-22 251022-177601001
==================
hp22mm, 采用新公式 R20= C4x12+c12， R21= C4x12+c11

2025-10-22 251022-177501001
==================
修改Hp22mmSCManager类中getSlotCount的bug，少了 ns &= 0x0F

2025-10-22 251022-177401001
==================
改 15=900，000
R16= 15,000,000

2025-10-22 251022-177301001
2025-10-22 251022-177201001
==================
Hp22mmSCManager类中downLocal函数做如下修改，取消B跟随P减锁的操作，而是按着标准头的方式减锁。以避免B的减锁与B的实际进度不一致，导致出现剩余次数出现锯齿形下降的问题

2025-10-22 251022-177101001
==================
1. Hp22mmSCManager类中，getLocalInk和getLocalInkPercentage函数中，P的放大倍率由5修改为7
2. downLocal函数中，增加一个
mHeads[mHeads.length - 1].mInkLevel -= c
以实现B的减记

2025-10-22 251022-177001001
==================
DataTransferThread类中的getInkThreshold函数的结尾处，如果是hp22mm打印头的场合，根据使用的喷嘴数，修正阈值（即阈值除以喷嘴数）

2025-10-22 251022-176901001
==================
Hp22mmSCManager类中的MAX_BAG_INK_VOLUME_MAXIMUM修改为 49600，即(15500 * 4)的80%

2025-10-22 251022-176801001
==================
取消251015-175801001版本中对Hp22mmSCManager的downLocal的修改，恢复减记P*时跟随减记B，但是当downLocal的dev参数指向B时跳过不减记（否则重复减记）

2025-10-20 251020-176701001
==================
hp22mm类中，清洗时的参数
R15=5000000(0x04C4B40)

2025-10-20 251020-176601001
==================
hp22mm类中，清洗时的参数
R15=48000(0x075300)
R16=45000000(0x2aea540)

2025-10-20 251020-176501001
==================
hp22mm.java类中的R15的参数恢复为
90M * 25 /(C1*C3) => 90M * 25 /(C1*C3*4)
并且修改清洗是的参数为
0x1388 -> 0x27100 (原值乘以32)

2025-10-20 251020-176401001
==================
hp22mm.java类中的R15的参数修改为
90M * 25 /(C1*C3*32) => 90M * 25 /(C1*C3)

2025-10-20 251020-176301001
==================
hp22mm.java类中的R15的参数修改为
90M * 25 /(C1*C3*4) => 90M * 25 /(C1*C3*32)

2025-10-17 251017-176201001
==================
修改251016-175901001的修改3的bug

2025-10-17 251017-176200001
==================
暂时将外部字符串的分隔符修改为^
public final static String TEXT_SEPERATOR         = "^"
(仅本版本有效)

2025-10-17 251017-176100001
==================
TxtDT类中，readLine之后的字符串，取消去除首尾空格的功能
//                line = line.trim();

2025-10-16 251016-176001001
==================
重新修改R16的计算公式，当C5=0且C6=0时 90M * C7 / C1 / 2； 当C5=0且C6=1时 R16 = C7 * C10 * 64 / C9 / 3.14
* 当C9=0时，按1取值
* 当C5=1或C6=0时，按C5=0且C6=0条件计算

2025-10-16 251016-175901001
==================
1. 插入U盘后，显示黄色提示窗提示U盘已插入
2. 升级IME时在与升级FPGA时显示进度相同的地方显示升级进度
3. 升级完FPGA，ko及IME后，提示重启的弹窗

2025-10-15 251015-175801001
==================
在Hp22mmSCManager类中的downLocal函数中，取消
Hp22mm.downLocal(0, c) （IDS）
因为
Hp22mm.downLocal(mHeads[dev].mID, c);
的最后一个元素应该已减记了IDS

2025-10-15 251015-175701001
==================
1. Hp22mmSCManager类中的Hp22mmHead子类中的
        volatile private int mInkLevel;
增加volatile属性，改善线程间的数据交换
2. ControlTabActivity类中的
...
2025-10-14 251014-175601001
2025-10-14 251014-175501001
==================
getInkThreshold函数中，当打印头类型为Hp22mm的时候，根据连供的逻辑为墨袋计算阈值。

2025-10-14 251014-175401001
==================
Hp22mmSCScheduler类中，有固定支持一个头修改为支持多头（启用mHead成员变量）

2025-10-14 251014-175301001
==================
修改hp22mm.java中的R18的計算公式，新公式为 Math.max((32 * ((25.4f * 2 * config.mParam[9]) / (3.14f * config.mParam[8])) / C3A), 1);
64 -> 32

2025-10-13 251013-175201001
==================
增加PinyinIME的升级机制，将PinyinIME.apk放置在U盘根目录下的IME目录中，通过测试页面的【UPD IME】按键启动升级

2025-10-13 251013-175101001
==================
修改PlatformInfo类中的getFPGAVersion函数中的修改遗漏

2025-10-11 251011-175001001
==================
1. 完善新的img构成机制，即只有M2和M9两种标准img，不同硬件的功能在此基础上更换ko来实现。具体：
   M2：只有一种ko，7000-7999
   M9：BAGINK专用ko，6000-6999
       BIGDOT专用ko，5000-5999
       HP22MM专用ko，4000-4999
       其余为标准SPI的M9专用ko，3000-3999
   本次修改的主要内容是：将用来显示的组合版本号与决定硬件种类的唯一码的获取分开，原来的getImgUniqueCode函数更名为getDispVersionCode，用于提供用于显示的版本号。原来的getImgUniqueCode函数修改为返回反应硬件种类的唯一码（为与原来代码兼容）
   当ko的版本号为
       6000-6999时，返回唯一码BAGINK
       5000-5999时，返回唯一码BIGDOT
       4000-4999时，返回唯一码HP22MM
2. 修改TestGpioPinsNew类中升级FPGA固件和ko的按键，移植画面相对下部，左右排列，按钮放大，目的是放置误按；并且点击按键时增加提示功能
3. 修改hp22mm.java中的R18的計算公式，新公式为 Math.max((64 * ((25.4f * 2 * config.mParam[9]) / (3.14f * config.mParam[8])) / C3A), 1);

2025-10-11 251011-174901001
==================
hp22mm.java类中的R15的参数修改为
90M * 25 /(C1*C3) => 90M * 25 /(C1*C3*4)

2025-10-7 251007-174801001
==================
修改A133版本apk的版本号显示逻辑，采用Aaaa_Bbbb_Cccc_Dddd的形式。具体内容参照PlatformInfo类中的说明

2025-9-30 250930-174701001
==================
恢复取消开机升级功能中的升级so部分，继续取消apk的升级

2025-9-29 250929-174601001
==================
1. 在保存编辑的信息文件后，增加sync操作
2. SmartCardManager中追加对一带二的支持

2025-9-28 250928-174501001
==================
msgImport和msgImportOnly在完成后关闭提示窗之前等待10s，以避免由于系统flush不及时而导致文件写入失败的问题

2025-9-26 250926-174401001
==================
通过设置时间对话窗升级的时候，增加等待插入U盘等待10秒的功能，检测到插入U盘后立即启动升级，否则报失败退出

2025-9-26 250926-174300001
2025-9-26 250926-174301001
==================
仅A133取消开机升级功能

2025-9-26 250926-174201001
==================
取消开机升级功能

2025-9-24 250924-174100001
==================
再放大信息显示提示窗的字体大小到48sp

2025-9-24 250924-174000001
==================
1. 修改250923-173900001的修改遗漏bug
2. 再放大信息显示提示窗的字体大小到40sp

2025-9-23 250923-173900001
==================
1. 替换字体7
2. 增大网络/串口接收信息显示提示窗字体大小
3. 对于模式5保存时的密码验证扩大到使用两个密码，任何一个密码正确即通过
4. 修改模式5的相应内容：
   4.1 Msg扩到到10个，分别为msg0-msg9，分别对应于DT0-DT9
   4.2 清单显示内容修改为：2# pieceNO DT0 DT1 DT2 DT3 DT4 DT5 DT6 DT7 DT8 DT9
   4.3 长按显示内容分为两列，因为一列显示不下10个DT值。但是分两列时，当出现并列两列宽度都很大时会横向显示不下
   4.4 由于此次修改前获取的msg为msg1-msg4，因此使用原来数据时，会没有msg0

2025-9-20 250920-173801001
==================
大字机的泵压循环功能，检查泵压的循环时间由10s改为5s

2025-9-20 250920-173701001
==================
修改250920-173601001版本遗漏

2025-9-20 250920-173601001
==================
22mm，临时修改apk下发给ko的数据格式
1 . 每列发544bit,    528数据+16bit0.
2.  双头是两个544bit,  实际没有区别
3.  整个数据的最后，有两544x4 个 全0列
【注意】需要与FPGA的568版配合使用

2025-9-19 250919-173500001
==================
2#连铸喷码机 => 2号喷码机

2025-9-18 250918-173400001
==================
修改250918-173300001版本bug

2025-9-18 250918-173300001
==================
250918-173200001版本修改打印完成后回送消息的参数格式，Dvc返回设备编号，而非RowCnt

2025-9-18 250918-173201001
2025-9-18 250918-173200001
==================
250918-173101001版本调整导入界面的按键配置

2025-9-18 250918-173101001
2025-9-18 250918-173100001
==================
在250915-172400001网络导入功能中，设备号使用参数58的值，取消从界面输入

2025-9-17 250917-173001001
2025-9-17 250917-173000001
==================
修改群组的选择逻辑，支持生成1个元素的群组

2025-9-17 250917-172901001
2025-9-17 250917-172900001
==================
增加保存编辑信息时验证密码的功能。具体内容如下：
1. 仅在快捷模式(参数P71)为MODE5时有效
2. 需要将密码文本编辑到pw.txt文件中，推送到/sdcard/system/pw.txt文件中。如无该文件，则使用缺省密码ok
3. 密码验证失败，或密码输入页面点击cancel文件将不会被保存
（追加了一个PWDialog4SaveMsg.java类，在EditTabSmallActivity的HANDLER_MESSAGE_SAVE_CONFIRM相应程序中追加调用该对话窗）

2025-9-16 250916-172801001
2025-9-16 250916-172701001
2025-9-16 250916-172601001
2025-9-16 250916-172501001
2025-9-16 250916-172500001
==================
1. 修改22mm的倒置处理逻辑。由原来的字节顺序倒置有apk处理，字节内顺序倒置有FPGA处理修改为全部在apk中处理。
   具体内容为:
   当打印头类型为HP22MM时，根据1头的倒置设置；当打印头类型为HP22MMX2时，1头倒置设置决定1头单独的倒置操作，2头倒置设置决定2头的单独倒置操作，3头倒置设置为12头合并倒置，并且同时忽略掉1头和2头的倒置设置
2. 取消R27[3]的倒置设置位

2025-9-15 250915-172401001
2025-9-15 250915-172400001
==================
为网络导入功能250910-171600001增加两个功能：
1. 在清单显示区长按某行时，弹窗显示该行的详细信息，包括（DT0-DT4，MSG1-4，STOVE)，然后，点击任何地方关闭显示
2. 增加检索输入框，输入pieceNo的后4为数字，查找该项目

2025-9-15 250915-172300001
==================
1. 修改64DOTONE喷头的允许操作，放开reverse和shift权限
2. 64DOTONE喷头偏移时不在除以24，其他头类型照旧
（辅助说明：倒置的设置1，2，3，4头分别设置时，各头分别倒置；如果12头同时设置倒置，则12头合并倒置，34头同理。如果1234头全部设置倒置，则1234头合并倒置（即整体倒置））

2025-9-12 250912-172201001
2025-9-12 250912-172200001
2025-9-12 250912-172101001
2025-9-12 250912-172100001
==================
修改250912-172000001bug

2025-9-12 250912-172001001
2025-9-12 250912-172000001
==================
1. 修改250911-171700001版本（有关网络功能）bug
2. 修改250911-171900001版本（有关hp22mmx2修改）bug

2025-9-11 250911-171900001
2025-9-11 250911-171800001
==================
修改hp22mmx2打印头类型时的编辑页面：
1. 显示编辑窗的中间横线
2. 高度选择从1-22，扩充为1-44

2025-9-11 250911-171700001
==================
250910-171600001的修改功能商榷版，暂时将启动专用页面放置到客户功能按钮中

2025-9-10 250910-171600001
2025-9-10 250910-171601001
==================
增加一个快捷模式5，用来从一个特定URL下载json文本，解析后赋值给多个DT。操作方法为：
1. 设置PP71为模式5，重启
2. 选定一打印任务后，开始打印，此时专用页面会弹出显示
3. 在专用页面中，自动显示上次保存的数据，如无数据显示区空
4. 在Row Cnt输入窗中输入喷码机号码，如需要2#喷码机的数据，则输入“2#”即可，然后点击Get Data按键，从云端下载json文本（需要有WIFI网络连接）
5. 下载数据后，如果获取到有效数据，则提取数据，更新到显示区，并且自动保存到Flash中；如果无法获取到有效数据，则弹出窗口显示错误提示
6. 选择某一列数据后，按着：
                printRowMsg1 => DT0
                printRowMsg2 => DT1
                printRowMsg3 => DT2
                ...
                printRowCnt => DT6
                id => DT7
    更新至数据桶，并且更新打印缓冲区后，更新至FPGA的ko中，等待触发打印。因此，需要在制作打印信息时，根据需要合理配置DT
7. 触发打印后，会自动向服务器发送保温通知
本次修改涉及：SystemConfigFile及ControlTabActivity类。及新增Server1MainWindow类，server1_data_layou, server1_data_list_item页面配置文件

2025-9-8 250908-171501001
2025-9-8 250908-171401001
==================
在pdPowerOn函数中，追加
...
2025-9-5 250905-171300001
2025-9-5 250905-171301001
==================
多处切换RFID和蓝牙的设置PI9的地方修改为，A133的话不做切换，因为A133中没有此功能；A20的话，保留切换

2025-9-4 250904-171201001
==================
在hp22mm.c的monitorThread中，暂时取消pd_get_temperature调用，会影响到循环变量i，从1变为0，只要调用底层的service_get_actual_temp就会出现这个问题，原因不明。可能是hp的库中对内存的操作影响的

2025-9-3 250903-171101001
2025-9-3 250903-171001001
2025-9-3 250903-170901001
2025-9-3 250903-170801001
==================
在monitorThread中，由于最初只考虑的是一个头，当有两个头的时候，第二个头永远不会被执行握手，导致60s后第二头断电

2025-9-2 250902-170701001
==================
A133平台取消开始打印时的
            startPhoEncTest();
            stopPhoEncTest();

2025-9-2 250902-170601001
==================
在SettingsTabActivity中增加一个ListView的onItemClick函数，主要目的是关闭不需要的输入法软键盘

2025-9-2 250902-170600001
==================
1. ExcelMainWindow类中修改错误的logo不显示错误进行了修改
2. 修改TestGpioPinsNew中升级FPGA固件进度的显示方法。改为分别显示升级进度和验证进度（从前只有升级进度）

2025-9-1 250901-170500001
==================
修改ExcelMainWindow类中修改图标的指定方法。由用户事先导入到pictures目录中，在excel文件中指定文件名（只支持bmp文件，在excel文件中可指定或不指定扩展名，指定扩展名时指定.bmp或.BMP均可）

2025-9-1 250901-170400001
==================
修改ExcelMainWindow类中clear命令的bug，没有清空mDataIndex.clear();

2025-9-1 250901-170301001
2025-9-1 250901-170201001
==================
与250829-169801001相同修改，为了暂时测试

2025-8-29 250829-170001001
2025-8-29 250829-170101001
==================
完善TestMain类中的execPhoEncTest函数

2025-8-29 250829-170001001
==================
在TestMain类中的execPhoEncTest函数中，如果mPhoEncTestDlg已经生成，可能是用户点击画面其他部分导致对话窗隐藏，这种情况下不重新启动测试，而是将隐藏的对话窗显示出来

2025-8-29 250829-169901001
==================
在升级了ko和so之后，增加执行sync，以确保升级成功（250819-168301001版本没有增加完全）

2025-8-29 250829-169801001
==================
暂时将FpgaGpioOperation类中的init函数中startPrint和设fpgaState的前后顺序对调（已经恢复）

2025-8-28 250828-169701001
==================
1. 重新梳理以下P94的值的意义：0-自动；1-强制RFID；2-强制SC；3-强制墨袋机
2. P94在参数设置页面修改为下拉菜单。名称为：Auto 自动，Rfid Rfid，SC SC，Bagink 墨袋机

2025-8-27 250827-169600001
==================
修改启动用户UserAPK的bug（修改A133版本的时候错误注销了）

2025-8-25 250825-169500001
2025-8-25 250825-169501001
==================
1. 修改hp22mmx2变量重叠的问题，原因是在DataTask类中的refreshVariable函数中，计算尺寸的时候，hp22mmx2没有修改，仍然还是使用hp22mm头类型的数据
2. 修改主页面activity_main中的两个为了放置扫描时弹出对话窗而设置的空间，由于没有设置底色，系统使用了灰色，导致出现1dp的白线，修改为与底色同色
3. 修改activity_main中时间的显示字体大小，由24sp -> 20sp。以避免在3.5寸屏上面出现日期文字底部缺失笔画的问题

2025-8-22 250822-169401001
==================
点按停止键之后，等待300ms再下发停止打印指令，以避免PG管脚争抢导致长鸣

2025-8-21 250821-169301001
==================
A133墨袋机，加墨保护间隔时间由2分钟改为1分钟

2025-8-21 250821-169201001
==================
A133墨袋机，开始加墨的修改为最近的10次测量当中，读到缺墨（PG9=1）的次数大于7次，以避免偶尔出现读数错误导致PG9=0时，不能继续加墨

2025-8-20 250820-169101001
2025-8-20 250820-169001001
==================
1. 取消250820-168901001版本中允许同号升级的功能
2. 优化了开机升级和时间设置页面的升级处理
3. 墨袋机加墨尝试次数由于加墨量改小了，从10修改为20

2025-8-20 250820-168901001
==================
【临时】250820-168701001版本的不禁止同号升级的版本

2025-8-20 250820-168801001
==================
恢复开机自动升级的逻辑到原来的逻辑。完善时间设置页面的升级处理

2025-8-20 250820-168701001
2025-8-20 250820-168601001
==================
1. 取消250819-168501001相同版本号可升级的临时功能，恢复原功能
2. 增加开机后检查U盘是否插入，持续10秒。这样可以避免部分A133插着U盘开机后，U盘插入事件不能及时检测到而影响正常升级的问题（如果识别事件超过10秒仍然无法识别升级）。同时带来的问题时，AllWinner图标消失后，DataLoading显示之前出现10s的黑屏

2025-8-19 250819-168501001
==================
临时允许相同版本号的apk升级，以测试升级不成功的情况

2025-8-19 250819-168401001
==================
与250819-168301001完全相同，只是为了做升级测试

2025-8-19 250819-168301001
==================
1. 在升级完apk后，执行shell的sync命令，然后睡眠时间从10s调整到20s再提示重启，增加复制文件被写入物理存储的成功率
2. 保存设置数据后，增加执行shell的sync命令，以争取保存到物理存储
3. 【临时】保存设置数据时，重复读写1000次，已确认是否真实写入成功

2025-8-18 250818-168201001
==================
修改播放报警音的处理逻辑，由于报警音播放使用的是newFixedThreadPool，这个线程池会将后续无法获得线程的处理放进队列，导致如果调用队列过长，会导致报警解除以后，仍然继续报警，直到等待队列全部处理掉。处理办法是如果线程占用则扔掉本次报警请求（由于正处于报警处理之中，扔掉也不会影响效果）

2025-8-18 250818-168101001
==================
1. checkPress函数中如果PG9读到高后关闭PH7等，否则之前如果有报警，报警灯就会一直亮着
2. checkPress函数中循环的调用增加判断压力是否够，不够不循环

2025-8-18 250818-168001001
2025-8-18 250818-167901001
==================
checkPress函数中增加读取PG9后=0时的log输出，已确认apk读到的值与img读到的值是否一致

2025-8-17 250817-167801001
==================
修改判断是否启动bagink功能的依据，不再根据img的种类是否为bagink，而是根据P94是否为0，为0时为bagink，其余值照旧。使用标准的m9img，标准m9img缺省支持bagink。

2025-8-14 250814-167701001
==================
墨袋机守护线程整理操作逻辑，加压时同时拉高PE4和PE5一秒，循环时拉高PE4一秒

2025-8-14 250814-167601001
==================
墨袋机守护线程修改判断PG9读值判断的逻辑，原来判断=1，实际应该为位移后的值

2025-8-14 250814-167501001
==================
墨袋机守护线程增加log输出，调试PE5不拉高的问题

2025-8-14 250814-167401001
==================
1. 取消250813-167301001增加的log输出，验证结果是由于计算数值过大，float会舍弃掉小数点后几位的数字，如2.3999882E7。这样就会出现虽然计算结果确实不同，但是不同部分被舍弃了。解决办法是，尽量不要采用太大的count
2. 增加墨袋机加墨时的错误次数显示

2025-8-13 250813-167301001
==================
临时版本，增加一点log输出，随后会取消。目的是验证count * mDTransThread.getKeptInkThreshold(i)的计算为什么在count很大时会出现两次打印才会减记的问题

2025-8-13 250813-167201001
2025-8-13 250813-167101001
==================
墨袋机加墨记录修改为记录多次加墨记录

2025-8-13 250813-167001001
==================
1. 墨袋机显示墨位状态的对话窗字体调小
2. 修正大字机检查压力逻辑缺陷(ControlTabActivity中的pressCheckInit和checkPress)

2025-8-12 250812-166900001
2025-8-12 250812-166901001
==================
恢复250724-165900001中去除的R5在双头时乘2

2025-8-12 250812-166801001
==================
1. 修改250811-166701001的bug，该bug导致墨袋机打印开始后RfidScheduler的schedule函数进入死循环
2. 修改22mmx2生成打印缓冲区时，与单头一样在每528字节数据之间插入528字节0

2025-8-11 250811-166701001
2025-8-11 250811-166601001
==================
1. A133墨袋机功能固化，取消临时性的强制BAGINK流程，改为标准形式判断img种类
2. 修改本机ip地址的显示位置

2025-8-8 250808-166501001
2025-8-7 250807-166401001
2025-8-7 250807-166301001
==================
增加A133的墨袋机读取状态的功能(临时版本)

2025-7-29 250729-166201001
==================
250728-166101001版本修改1功能中，追加一些log输出

2025-7-28 250728-166101001
2025-7-28 250728-166100001
==================
1. 增加大字机的泵压循环功能，在ControlTabActivity中借用心跳功能调用本功能
2. 增加一个ExtGpio获得版本号的功能，并且附加在当前FPGA版本号的后面（两位数字）

2025-7-26 250726-166000001
==================
1. Hp22mmNozzleSelectDialog中，互换IDS1(等于界面的IDS2）和IDS0(等于界面的IDS1）的mask值，因为有两者互相错位了的报告
2. 取消P80和P82参数，在Hp22mm的R23和R24中，不再参照该参数，而是设固定值0
3. P80修改为泵压循环（PumpCircu），只是strings.xml做了修改，没有没实际填充功能

2025-7-24 250724-165900001
==================
hp22mm双头的时候R5暂时不乘2

2025-7-23 250723-165800001
2025-7-23 250723-165700001
==================
hp22mmx2的横向放大倍率调整为x1的两倍

2025-7-22 250722-165601001
2025-7-22 250722-165600001
==================
250721-165500001中对于QR码的处理扩展至DM，Gs1QR及Gs1DM中

2025-7-21 250721-165500001
==================
当打印头是hp22mm或者hp22mmx2时，生成打印缓冲区时宽度和高度缩小的倍率不一样，导致生成的二维码会按着短边生成一个缩小的图，修改为按着短边生成的图放大至全空间

2025-7-21 250721-165401001
2025-7-19 250719-165400001
==================
22mm，开始打印前的循环改为2秒

2025-7-17 250717-165300001
==================
1. 在250717-165200001的基础上，用户点按确认后，先下发4K的0
2. 条码功能，在BaseObject设置高度的函数中，取消最小值4.0的限制，并且在MessageObject类中，mBaseList中追加一个更小的0.25选项

2025-7-17 250717-165200001
==================
修改250716-165000001增加的，扫描协议7，修改扫描内容对话窗点按确认后，延时3秒后的显示方式，修改为全屏白色衬底显示黑色倒计时

2025-7-17 250717-165100001
==================
在RFIDDevice.java和N_RFIDDevice.java类中，永久取消2023-5-18增加的对于Bagink机型的锁值，mFeature[6]要介于64-164之间的限制

2025-7-16 250716-165000001
==================
扫描协议7，修改扫描内容对话窗点按确认后，延时3秒，显示转轮和倒计时数据

2025-7-14 250714-164901001
==================
1. A133平台，HP22MM版型时，烧写FPGA新固件不再先拉低PI7，完成后拉高。其余场合照旧
2. HP22MM时的IN3从PG5修改为PH26

2025-7-11 250711-164801001
==================
在IDS_Init函数中，开启ADC的压力监测模式工作

2025-7-11 250711-164701001
2025-7-11 250711-164601001
==================
...
2025-7-10 250710-164500001
2025-7-10 250710-164501001
==================
增加一个参数60的值，当=5678时，输出log。当=0时，不输出log
【注意】以前版本是始终输出log，本次修改后，如果不做任何设置，缺省是不再输出log，只有当参数60设置为5678时，才会输出log

2025-7-10 250710-164401001
==================
1. 在BarcodeObject类中的makeBinBitmap函数中，判断如果是动态条码二维码，或者虽然是静态条码，但是内容为超文本，则不预生成图案，以免在打印时还得清除
2. 在DataTask的refreshVariable函数中，生成条码图案（动态条码，或者含超文本的静态条码）后，合并原内容复制，如果覆盖复制的话，可能会将纵向有重叠的先前内容删除掉

2025-7-10 250710-164301001
==================
修改BarcodeObject中生成DM码的log输出，验证一下Content。

2025-7-8 250708-164200001
==================
1. 修改编辑页面保存是弹出的保存，保存后打印按键的配置信息，当内容比较多（如法语），会导致按键变形下移
2. 修改xls页面的搜索功能，由原来的精准查找改为模糊查找，显示区不在高亮显示精准命中的项目，而是只显示命中的一组数据

2025-7-7 250707-164100001
==================
完善法语字符串的内容

2025-7-7 250707-164000001
==================
xls导入页面增加退出按键

2025-7-7 250707-163900001
==================
1. 修改数个法语字符串的内容
2. 修改ITF-14画外框的算法，右下变向内移动一个单位，否则右下边少画一条，导致变窄
3. 修改xls的logo画法，原画法不能画出正确的大小（DataTask/refreshVariable中的instance of GraphicObject）

2025-7-7 250707-163800001
==================
修改250627-163500001版本中对LOGO_BITMAP的未复制的修改遗漏，并且完善了打印记录的记录和导出

2025-7-4 250704-163700001
2025-7-4 250704-163701001
==================
hp22mm，输出口Output4由PG3改为PI9

2025-7-3 250703-163601001
==================
修改输入输出管脚的配置，对应于HP22MM和标准SPI，配置不同。修改为剑TestGpioPinsNew.java

2025-7-1 250701-163501001
==================
在SettingsListAdapter类中，取消C77设置结果的有效性
				if((msg.arg1 & 0x0F) == 0x00) msg.arg1 |= 0x01;
以避免进设置Pen2的时候，取消所有Pen1的slot仍然会出现slot0被选上的情况

2025-6-27 250627-163500001
2025-6-27 250627-163400001
==================
增加xls文件导入处理的启动标识，启用P71参数，增加MODE4。当为MODE4时，根据需要更新LOGO的GraphicObject内容（直接从ExcelMainWindow中获取Logo的Bitmap，然后二值化后粘贴）

2025-6-27 250627-163300001
2025-6-26 250626-163200001
==================
【开发中版本】增加xls文件导入及相关处理

2025-6-23 250623-163100001
==================
更换繁体中文的资源文件

2025-6-19 250619-163000001
==================
A20临时尝试设置RFID休眠，尝试新的方法。在SmartCardManager和Hp22mmSCManager类中，初始化的时候，调用停止RFID发射的功能，使得rfid不在发射无线信号

2025-6-18 250618-162900001
2025-6-18 250618-162800001
2025-6-18 250618-162801001
==================
A20临时尝试设置RFID休眠，看看是否奏效

2025-6-13 250613-162701001
2025-6-13 250613-162601001
==================
临时按着标准hp22mm的规格生成打印缓冲区（高=1056，不填0；循环时的参数是：高=528，528字节后跟528字节0）（输出版本后及时恢复）

2025-6-11 250611-162501001
==================
暂时恢复250607-162201001的修改2

2025-6-11 250611-162401001
==================
取消250611-162301001的修改2

2025-6-11 250611-162301001
==================
1. 增加在高频打印的时候停止log输出的功能
2. 暂时在A133版本中开始打印的时候清除Enc和Pho的操作（尝试一下看看效果，可能是因为这个操作导致A133的hp22mm不能开始打印）
3. 隐藏扫描协议8和9，作为保留项

2025-6-7 250607-162201001
==================
1. 升级apk后等待3秒改为等待10秒，否则有些设备会出现升级无效的情况
2. 暂时取消FPGA的内存log输出和FPGA反应速度测试的3096条log输出（解决停止打印时出现长鸣的问题）

2025-6-6 250606-162100001
==================
A20版apk，追加扫描协议10。扫描客户二维码，将 $ 符号前面的内容赋给DT0

2025-6-6 250606-162101001
==================
从测试内存问题的临时版本恢复到正常版本

2025-6-3 250603-161700001
2025-6-3 250603-161701001
==================
注释掉NativeGraphicJni中的log输出，并且将测试版PrintTask函数中的睡眠时间修改为6ms

2025-5-30 250530-161300001
2025-5-29 250529-161200001
2025-5-29 250529-161100001
2025-5-29 250529-161000001
==================
完善250529-160900001功能

2025-5-29 250529-160900001
==================
增加扫描协议8和9

2025-5-29 250529-160801001
==================
取消250528-160701001版本追加的疲劳试验，恢复正常版本，但保留忽略RFID检查，仅供测试

2025-5-28 250528-160701001
==================
250528-160601001作废。为A133-M9定制疲劳试验apk

2025-5-28 250528-160601001
==================
250528-160501001作废

2025-5-28 250528-160501001
==================
【临时版本】仅高速循环生成打印数据

2025-5-28 250528-160400001
==================
临时取消开始打印时加热的功能

2025-5-27 250527-160301001
==================
ids.c中的i2c设备，A20是i2c-1，A133应该是i2c-2

2025-5-27 250527-160201001
==================
修改uart.c的bug，在uart_recv函数中，设置timeout值的时候，tv_usec不能大于1秒(1000000)，原来的判断在等于1秒的时候会出现这种情况。1599-1601版本的修改恢复

2025-5-27 250527-160101001
==================
查找uart.c中select发挥22号错误的原因

2025-5-27 250527-160001001
==================
uart.c中添加一些log

2025-5-27 250527-159901001
==================
修改uart.c中打开串口的逻辑，由于A133使用串口ttyS7，而不是A20的ttyS3，因此需要考虑在A20和A133时打开不同的串口

2025-5-26 250526-159800001
==================
hp22mm.c中，pd_set_temperature_override改在上电之前设置

2025-5-24 250524-159700001
2025-5-24 250524-159600001
2025-5-24 250524-159500001
==================
Hp22mm.java, 调整循环参数下发

2025-5-24 250524-159400001
2025-5-24 250524-159300001
==================
1. 恢复在ko中writeFlash时，每次执行spi_transbuf中的spi_sync前后设置PI4的高低，取消由TestGpioPinsNew类中在writeFlash的最前面拉低，完成后拉高PI4的操作
2. (hp22mm)R25参数扩充为：Bit[18] = 1(循环)/0(普通); Bit[17:2] = 循环参数; Bit[1:0] = 1(打印)/0(停止)

2025-5-23 250523-159200001
==================
取消 monitorThread 中 pd_set_recirc_override 的调用

2025-5-23 250523-159100001
==================
增加max_freq, recovery_time, between_pages_time值的输出

2025-5-22 250522-159000001
2025-5-22 250522-158900001
==================
在 monitorThread 中增加 pd_set_recirc_override调用，设为recovery，200%

2025-5-22 250522-158800001
==================
取消 monitorThread 中的 pd_set_temperature_override 调用
（经过实验证明 pd_set_temperature_override 函数与 pd_set_over_energy_override 都会返回 Cannot be executed ，错误号为14号，因此调用无效）

2025-5-22 250522-158700001
==================
在monitorThread函数中追加pd_set_over_energy_override的地方，换成pd_set_temperature_override，设置35度，看看能否执行成功

2025-5-20 250520-158600001
==================
在monitorThread函数中追加pd_set_over_energy_override

2025-5-20 250520-158500001
==================
暂时关闭所有set_temperature_override

2025-5-20 250520-158400001
==================
避免250520-158000001修改报警

2025-5-20 250520-158300001
==================
恢复到250520-158000001版本的功能

2025-5-20 250520-158200001
==================
恢复250520-157900001版本的修改内容

2025-5-20 250520-158100001
==================
1. 取消250520-158000001修改
2. 暂时取消250518-157201001之后的对于循环功能的修改

2025-5-20 250520-158000001
==================
在monitorThread中，追加一个 pd_set_temperature_override 调用，以确认该调用是否每次都会被拒绝执行

2025-5-20 250520-157900001
==================
在hp22mm.c的monitorThread函数中，追加一个 pd_get_temperature_override 调用，以确认设置的目标温度

2025-5-19 250519-157800001
==================
取消250519-157300001 -> 250519-157700001的临时性修改，固化250519-157400001版本增加的循环功能，只在打印前实施循环，不在清洗前执行循环

2025-5-19 250519-157700001
==================
暂时取消报警

2025-5-19 250519-157600001
==================
恢复ids的初始化

2025-5-19 250519-157500001
==================
修正250519-157300001，同时取消ids和pd的初始化

2025-5-19 250519-157400001
==================
1. 增加P81，作为循环的参数设置。固化P80（ColumnSpacing）和P82（SlotSpacing）
2. 在清洗或者打印流程开始前，增加一个循环操作，具体操作内容参见Hp22mm类中的hp22mmCirculation函数

2025-5-19 250519-157300001
==================
暂时取消pd的初始化，直接返回成功，以方便测试清洗功能

2025-5-18 250518-157201001
==================
根据A133的版型，如果是M2的则RFID的串口为ttyS2，否则ttyS7。通用串口，M2无，其余为ttyS2

2025-5-16 250516-157100001
2025-5-16 250516-157000001
==================
临时在528字节基础上加528字节0

2025-5-15 250515-156900001
==================
hp22mm, Reg4=33, Reg5=132

2025-5-15 250515-156800001
==================
hp22mm, Reg4=33, Reg5=66

2025-5-15 250515-156700001
==================
修改250515-156600001的bug

2025-5-15 250515-156600001
==================
对应于cycling标准修改hp22mm。暂时修改的内容是：
1. 列高由1056 -> 528
2. print_head_driver.c :: pd_get_print_head_status中的系统服务调用返回值增加16字节返回值的合法性

2025-5-10 250510-156500001
==================
修改PCCommandHandler类中WriteFiles函数中buffer生成长度，原来长度为8192，并不是实际需要的长度，导致接收数据时发生错误

2025-5-8 250508-156401001
2025-5-7 250507-156301001
==================
（仅对A133有效）在开始打印时超频，停止打印后恢复正常

2025-5-7 250507-156201001
==================
暂时取消250428-156001001的修改

2025-4-29 250429-156100001
2025-4-29 250429-156101001
==================
在DataTransferThread类中的PrintTask的run函数中，增加一个记录从empty事件发生到下发数据之间时长（实际上就是生成数据所用的时间），并且打印的功能

2025-4-28 250428-156001001
==================
在开始打印前清理PhoEnc之前，如果是A133平台就将机器状态修改为10（正常为11）。用为A133平台开始打印前会直接跳数一次，可能是PhoEnc没有清理干净，这个修改就是为了使得清理后不在出现PH14或者PH15

2025-4-27 250427-155901001
2025-4-27 250427-155801001
==================
完善导入导出操作

2025-4-22 250422-155701001
==================
1. 在MainActivity的onCreate中，增加了
		FpgaGpioOperation.mFd = 0;
		RFIDDevice.mFd = 0;
		ExtGpio.mFd = 0;
    这部分代码的意图是，有时候会发生Activity重建的情况，如果不清0，会使用原来的实例，导致访问错误
2. 在ControlTabActivity盒SettingsTabActivity中的layout文件中，明示低增加背景颜色设置
    android:background="@color/background"
    否则，A133的系统会出现切换页面时有前面页面的残留

2025-4-16 250416-155600001
==================
A133
1. 取消250416-155500001的修改
2. 修改A133的升级方法，回归操作方法与A20的操作方法一致：
   2.1 apk的升级
       将Printer.apk文件放置在U盘的根目录，方法不变
   2.2 so的升级
       so的升级跟apk走，但是由于apk需要先升级，因此会出现两次提示升级完成需要重启的提示，重启两次
   2.3 ko的升级
       在测试页面升级，方法不变

2025-4-16 250416-155500001
==================
A133专用测试apk启动设PE9为高（以内PE9是输入端口，应该无效，测试目的是排除apk是否在启动时主动将PE9拉低了，因为测试显示apk启动后PE9变低了）

2025-4-10 250410-155400001
==================
改进250410-155200001修改2的计算公式

2025-4-10 250410-155300001
==================
修改250410-155200001修改2的bug

2025-4-10 250410-155200001
==================
1. 在apk中增加对FGZJ，FNSM两款对应新显示屏的img的支持
2. hp22mm.java类R18计算式中，config.mParam[2]取300整数倍

2025-4-10 250410-155100001
==================
由于测量有误，取消掉250408-155000001的修改。
调整M9Test的页面布局，使得各部件间更加紧凑，否则3.5寸屏显示不下

2025-4-8 250408-155000001
==================
R6X48, R6X50, E6X48, E6X50,E5X48, E5X50在计算Threshold是点数减半（因为实测中发现减锁量过大，减到实际墨量的202%才合适）

2025-4-3 250403-154900001
==================
在M9-Test页面追加PG589的测试功能

2025-4-1 250401-154800001
==================
修改支持新新屏版本的img（主要是屏幕旋转和亮度调整方面的功能），img的识别符号为FNG3（F代表新新屏，N新板，G代表GPIO版本非SPI，3代表300dpi）

2025-4-1 250401-154700001
==================
无修改，明确一下之前的修改都包含进去了

2025-3-29 250329-154600001
==================
在250329-154500001修改2的基础上，根据上次打印的剩余次数占比来调整本次打印的剩余次数（如上次的Threshold=5，打印了3次，应该剩余2次。此时更换到Threshold=30的新任务后，该新任务的开始剩余次数不再是30次，而是30的2/5次，即12次）

2025-3-29 250329-154500001
==================
1. DataTransferThread类中的initCount函数中，mcountdown[i]的初始值由原来的0修改为threshold值。否则切换打印任务时该值会被设为0，导致减锁
2. 修改导入导出对话窗的旁白，以使3.5寸屏也能显示全案件中的字符

2025-3-24 250324-154400001
==================
修改修改BarcodeObject类中的createCodeBitmapFromDraw中计算字体高度和drawText函数y值的计算方法

2025-3-24 250324-154300001
==================
1. 只要有扫描或者网络命令传入就Toast显示版本原文，无论是合法报文还是非法报文。目的是为了调试方便，避免出现错误时画面没有任何显示的情况出现
2. 修改BarcodeObject类中的createCodeBitmapFromDraw中计算横向方法倍数的方法
   由 paint.setTextScaleX(2);
   修改为 paint.setTextScaleX(1.8f * width / height / content.length());
3. 修改InkManagerFactory类和InkSchedulerFactory类中根据C94指定SC或者RFID的方法， 增加SmartCard.exist函数的调用，因为这个函数会传递一些环境参数

2025-3-18 250318-154200001
==================
1. 更新4字体
2. 临时增加一个切换RFID和SC的参数，使用参数94。当C94=1时，选择RFID；当C94=2时，选择SC；当C94=0时按原来逻辑执行

2025-3-17 250317-154100001
==================
修改蓝牙的功能。由原来仅使用串口蓝牙，增加为可以分别使用串口蓝牙或者RTL8723DU模块。
1. 暂时取消BLE目录下的蓝牙功能，整合到Bluetooth目录下，重新修改了实现机制
2. 通过将原来的BLEDevice的功能，整合入BLEServer类，支持串口蓝牙模块，数据交互接口调整至Bluetooth目录下的BLEStreamTransport类
3. 增加新的BTServer类，用来支持RTL8723DU，这个模块使用的是传统的蓝牙命令（不是BLE），数据交互接口使用标准的StreamTransport类
4. BLEServer和BDServer继承自BluetoothServer类
5. BLE目录暂时保留，作为BLEServer类不成熟时的参考，待BLEServer类成熟以后，可以删除该目录
（新功能需要支持RTL8723DU驱动的新的img的配合）

2025-3-14 250314-154000001
2025-3-14 250314-153900001
2025-3-14 250314-153800001
==================
尝试修改加墨线程干扰的问题（可能原来的CachedThreadPool会有休眠时被删除的问题？，改为使用普通的线程）

2025-3-14 250314-153700001
==================
1. 修改250313-153600001的bug，追加
2. 增加开阀前，开阀中和关阀后的取值，格式变为:
RPNHHMMQ
R:开阀前状态。1(PEn高), 0(PEn低)
P:开阀中状态。1(PEn高), 0(PEn低)
Q:关阀后状态。1(PEn高), 0(PEn低)

2025-3-14 250314-153600001
==================
SmartCardManager类中，追加在addInkOn执行后检查一下PE4，PE5是否真实设置上了。显示在加墨记录中，加墨记录的每个元素的格式变为：
RNHHMM (每个代表一位)
R: 1(PEn设置成功), 0(PEn设置失败)
其余项目参照250312-153200001修改3的内容说明

2025-3-13 250313-153500001
==================
修改250312-153200001中修改4的bug

2025-3-13 250313-153400001
==================
修改250312-153200001中修改3的bug

2025-3-13 250313-153300001
==================
1. 修改com_smartcard.c中的exist函数，增加一个参数，用来区分是CB2平台还是CB10平台，两个平台的i2c组别不同（CB2使用i2c1，CB10使用i2c2）
2. 通知修改Smart Card.java中的exist函数参数

2025-3-12 250312-153200001
==================
1. 扩大2020-11-13的修改，将秒是否变化也纳入定期检查范围
2. 修改秒的取值方法
3. DATA_SOURCE_SCANER7的分隔符由#修改为$
4. 修改SmartCardManager，增加加墨的记录。格式为[NHHMM,]，其中N代表头号(1,2,...)，HHMM为启动加墨的时分。最多收录20次的加墨记录

2025-3-7 250307-153100001
==================
在测试页面增加一个自动开关阀试验，名称为"Auto SmartCard Valve On/Off Test"。执行逻辑是开始测试以后，每隔10秒开阀一次，持续1秒

2025-3-6 250306-153000001
==================
增加22mm长清洗时可以选择清洗头的功能

2025-3-5 250305-152900001
==================
修改22mm清洗时下发参数为固定值

2025-2-27 250227-152800001
2025-2-27 250227-152700001
==================
修改Hp22mmSCManager类中，当C77没有选任何头时mHead=null的死机问题。-> 仅管理IDS

2025-2-27 250227-152600001
==================
增加22MM时的一带二，此时C31=HP22MM，C77=两个打印头，C38=12。下列情况下报错：
1. C31=22MMx2, C77不等于两个头，报错，头太少
2. C31=22MM，C77=两个头，C38不等于12时，报错，头太多
3. C31=22MM，C77不等于两个头，设置了C38=12，报错，头太少
4. C31=22MM，C77不等于Pen1也不等于Pen2，报错，头太少
【注】 22MMx2的头，设置了C38=12，不报错，但C38的设置被忽略

2025-2-24 250224-152400001
==================
1. 修改com_smartcard.c当中readRTC函数中SC_I2C_DRIVER_read返回-1时的错误处理缺失
2. 修改hp22mm.c及Hp22mm.java中的部分逻辑，使得其支持ids初始化成功后立即启动守护线程和加压
3. 恢复C31和C77相互验证设置合理性，当出现不合理的设置组合时报警

2025-2-24 250224-152300001
==================
修改250224-152200001版本bug

2025-2-24 250224-152200001
==================
大幅修改hp22mm机型对于多头分别选择时的处理逻辑：
1. 参数30的打印头的选择只影响生成打印数据的每列高，参数77的打印头选择控制打印头数量
2. 主页面的打印头显示，当参数77只选择Pen1时，只显示P1和B的数值，只选择Pen2时，只显示P2和B的数值，选择Pen1和Pen2时，显示P1，P2和B的数值，
3. 在Hp22mmSCManager类中，增加一个Hp22mmHead类，用来管理各个头的状态
4. so库做相应的修改，具体内容可参考hp22mm.c中的注释

2025-2-21 250221-152100001
==================
22mm清洗时，R20，R21固定为0

2025-2-21 250221-152000001
==================
更换noslant.bin文件

2025-2-21 250221-151900001
==================
短清洗也根据打印头类型使用noslant

2025-2-21 250221-151800001
==================
长清洗使用的bin，使用noslant.bin的条件取反

2025-2-21 250221-151700001
==================
打字机清洗的时候，S5再乘2，S5=2400

2025-2-21 250221-151600001
==================
打字机清洗的时候，S5=1200，其它类型头时，S5=600

2025-2-21 250221-151500001
==================
1. ControlTabActivity类中的refreshInk函数中，22mm当出现INVALID时，显示的地方没有修改，补上修改
2. 长清洗提示窗的内容写5分钟

2025-2-20 250220-151400001
==================
长清洗打字机根据打印头的类型分别使用不同的bin文件。
	MESSAGE_TYPE_16_DOT || MESSAGE_TYPE_32_DOT || MESSAGE_TYPE_32X2 || MESSAGE_TYPE_16DOTX4 || MESSAGE_TYPE_64DOTONE 时，使用noslant.bin
其余的打字机打印头使用原来的purge4big.bin

2025-2-20 250220-151300001
==================
250220-151100001版本中R20及R21的计算公式，修改为
... 24/6  -> ... 12/6。

2025-2-20 250220-151200001
==================
hp22mm.c代码中，给几处调用pd_sc_read_oem_field的函数入口增加了log输出，确认导致pd_sc_read_oem_field执行时发生14号错误的原因是什么

2025-2-20 250220-151100001
==================
修改250217-150000001版本R20及R21计算公式错误。

2025-2-20 250220-151000001
==================
修改22mm是clean的持续时间，从当前的2分钟修改为2分钟的2.17倍

2025-2-20 250220-150900001
==================
修改250217-150000001版本R20计算公式错误。

2025-2-19 250219-150800001
==================
1. 确定在22mm的时候，短清洗数据缓冲区放大36倍 -> 1倍，即不放大
2. 追加22mm墨盒寿命显示，支持多墨盒和墨袋。修改的内容包括：
    2.1. ControlTabActivity中refreshInk函数中的相应地方
    2.2. Hp22mmSCManager中的mInkLevel修改为mInkLevel[3]，分别代表PEN0, (PEN1，)IDS，以及相关的操作的地方
    2.3. hp22mm.c，支持getLocalInk和downLocal函数中支持不同头的操作
    2.4. PEN的最大值为IDS最大值的5倍

修改逻辑是：
【原逻辑】
当downLocal的时候，减记相当于打印头开启的喷嘴数量的值（即如果开启了两个喷嘴，每次减记时减记2），并且保存在IDS的OEM区，不考虑PEN的寿命。
【修改后的逻辑】
当downLocal的时候，减记相当于当前打印头开启的喷嘴数量的值，并且保存在相应的PEN的OEM区，同时减记IDS的OEM区并且保存。

2025-2-18 250218-150700001
==================
【临时版本】短清洗数据缓冲区放大36倍 -> 1倍，看看22mm短清洗的表现再确定

2025-2-18 250218-150600001
==================
【临时版本】短清洗数据缓冲区放大36倍 -> 4倍，看看22mm短清洗的表现再确定

2025-2-18 250218-150500001
==================
修改250217-150000001的bug，当倒置时R27[3] = 1, 错误的设成了R27[2] = 1

2025-2-18 250218-150400001
==================
修改TxtDT类中的bug，问题出在control3_frame定义文件中，在2024-12-28修改增加在打印过程当中显示计数器当前数值的功能时，将预览画面遮盖住了。

2025-2-18 250218-150300001
==================
修改250217-150000001版本的bug

2025-2-18 250218-150200001
==================
注释掉ids.c中的注释，已经service.c中有关UART的注释，以减少log输出量。修改守护线程的休眠时间
#define POLL_SEC 1 -> 2
#define INK_POLL_SEC 2 -> 4
目的：减少log输出量，增加log可读性，减少守护线程占用CPU时间

2025-2-17 250217-150100001
==================
修改后增加的控件ctrl_hp22mm_result_area（及以下控件）只增加到了control_frame中，而没有追加到control2_frame,control3_frame,control4_frame中的问题

2025-2-17 250217-150000001
==================
1. 清洗时，R17和R19均设0（hp22mm.java类）
2. 修改R20，R21计算公式为：R20= C4x(C10x2/(C9x3.14*4))/+(c11*24/6)； R21= C4x(C10x2/(C9x3.14*4))/+(c12*24/6)
   (注：C11和C12好像是用反了，C11给R21，C12给R20)
3. 上电后停止加热，只有开始打印后再加热
4. 22MM和22MMX2两种类型的打印头均支持镜像和倒置（倒置处理暂时在apk中的DataTask类和BufferRebuilder类中实现，以后考虑移至img中）
5. 墨盒墨袋雨量显示问题(待修改)
6. 打印中切换信息的问题 ： 我这里测试是能够切换的。

2025-2-13 250213-149900001
==================
22mm时，清洗时，长清洗维持时长改为120秒

2025-2-13 250213-149800001
==================
22mm时，清洗时，R15=5000

2025-2-13 250213-149700001
==================
1. 长清洗使用hp22mm_purge.bin。大小为1501*1056
2. 短清洗使用hp22mm_purge_short.bin。大小为16*1056
3. 长清洗维持时长改为90秒

2025-2-13 250213-149600001
==================
更换更大的hp22mm_purge.bin。看看clean时的喷墨量

2025-2-12 250212-149500001
==================
Reg15 = 3750; Reg16 = 3825000

2025-2-12 250212-149400001
==================
对于241024-135200001版本中过滤掉多余的PH14中断的处理，在4FIFO类型img中不执行此操作，如果执行此操作，会发生不打印的问题

2025-2-10 250210-149300001
==================
250210-149200001中的Reg15修改为：
Reg15 = 90M / 15K

2025-2-10 250210-149200001
==================
250210-149000001中的Reg15修改为：
Reg15 = 90M / 50K

2025-2-10 250210-149100001
==================
短清洗的持续时长延长到3秒

2025-2-10 250210-149000001
==================
22mm清洗时：
1. 固定设置Reg15 = 90M / 10K
2. 固定设置Reg16 = 90M / 10
...
2025-2-10 250210-148900001
==================
将清晰的频率修改为每5ms检查一次下发数据请求

2025-2-10 250210-148800001
==================
长清洗时长修改为2min

2025-2-10 250210-148700001
==================
修改短清洗时的bug

2025-2-8 250208-148600001
==================
修改C63在22mm或者非22mm时检查最导致的处理bug

2025-2-8 250208-148500001
==================
短清洗和长清晰功能的修改（2025-2-7实施）。
1. 对于短清洗，开启清洗后每100ms查询一次empty，如果发生empty就下发数据，持续1秒。因此，理想状态下会下发10次
2. 对于长清洗，开启清洗后每100ms查询一次empty，如果发生empty就下发数据，持续5秒。因此，理想状态下会下发50次
请确认清洗的喷墨状况，是否符合预期。这个功能对于非22mm的机型也适用。不同机型使用的清洗bin不同，执行逻辑相同

2025-2-7 250207-148400001
==================
C63参数修改设置框的最大值限制到,当为22mm类型时为55，其它类型为40。修改场所为SystemConfigFile类中的checkParam函数中

2025-2-7 250207-148300001
==================
暂时修改PowerOn函数，增加一个设定温度的参数

2025-1-21 250121-148200001
==================
250121-148100001改进，C1=1000，C7=200

2025-1-21 250121-148100001
==================
为Hp22mm.java类中的getSettings增加一个参数，用来区分当前生成的参数是为了打印还是清洗，当为数据时，使用C1和C7的内容，按着C1=2000，C7=400来处理

2025-1-20 250120-148000001
==================
暂时将R23和R24设为原值的一倍
    regs[REG23_COLUMN_SPACING] = C80
    regs[REG24_SLOT_SPACING] = C82

2025-1-20 250120-147900001
==================
1. 暂时将R23和R24设为原值的一倍
    regs[REG23_COLUMN_SPACING] = 4  => 8
    regs[REG24_SLOT_SPACING] = 52 => 104
2. 修改250120-147700001版本的bug

2025-1-20 250120-147800001
==================
暂时将R23和R24设为原值的一半
    regs[REG23_COLUMN_SPACING] = 4  => 2
    regs[REG24_SLOT_SPACING] = 52 => 26

2025-1-20 250120-147700001
==================
修改C31参数选择hp22mm或者hp22mmx2时与C77参数中打印头指定匹配情况的处理逻辑。
1. 当C31指定为hp22mmx2类型时，必须是两个头工作，因此无视C77的设置内容，按双头操作
2. 当C31指定为hp22mm类型时，如果C77选择了双头，则返回错误，初始化失败，在主页面显示弹出窗信息
    Too many heads indicated in C77
3. 当C31指定为hp22mm类型，并且C77也指定了单头时，如果两者不匹配，会导致配对失败，此时初始化失败，在主页面显示弹出窗信息
    Pairing failed. Please check C77 head setting
4. 其它初始化错误照旧，按底层返回错误信息返回，但增加在主页面显示弹出窗信息
5. 喷嘴的选择不受头的影响，只根据C77的内容设置

2025-1-19 250119-147600001
==================
hp22mmx2的高度按2112点(1056*2)计算

2025-1-19 250119-147500001
==================
增加一个hp22mmx2的打印头类型，用来决定22mm打印时下发参数中的R5和R11的值。
hp22mm类型时
    R5=132， R11=0
hp22mmx2类型时
    R5=264， R11=1

2025-1-18 250118-147400001
==================
修改22mm寄存器(R11)的值，当一个打印头时为0， 两个打印头时为1

2025-1-18 250118-147300001
2025-1-18 250118-147200001
==================
临时在DataTransferThread类中的purge函数中追加一个根据打印头为22mm的时候，启用转为22mm准备的打印文件下发清洗，仅仅为了临时测试，后续需要验证和完善

2025-1-18 250118-147100001
==================
...
2025-1-17 250117-147000001
==================
再GraphicObject类中，取消setWidth为空函数，因为这样会固定图片的宽度，设置该函数调用母类的函数，2024-10-28修改为空函数时的目的是防止按着路径的字符串计算变量的宽度（路径保存在mContent当中），这次没有复现该种情况，可能原来是受BaseObject中setContent后重新计算宽度的影响，经确认BaseObject的setContent函数好像不再重新计算宽度了，似乎不再受此影响。
经过确认生成修改保存都没有问题，重新读进来后也没有问题

2025-1-17 250117-146900001
==================
修改Java_com_purge的清洗方法，改为两个slot同时清洗

2025-1-16 250116-146800001
==================
修改purge_complete_mark_oem的定义，原来错误的定义为bool了，因该是uint32_t型(因为是保存在PD_SC_OEM_RW_FIELD_1中），这个bug会导致只能读到0或者1

2025-1-16 250116-146700001
==================
...
2025-1-16 250116-146600001
==================
取消250116-146500001版本中的mutex设置，根据hp的menu.c中的注释说明，应该不需要

2025-1-16 250116-146500001
==================
...
2025-1-15 250115-146400001
==================
取消250115-146300001的临时修改。
...
2025-1-15 250115-146300001
==================
临时取消RFID的检查

2025-1-15 250115-146200001
==================
hp22mm.c中，取消规避ERROR_DEVICE_SEEN_BEFORE错误，恢复原样。同时对common.c中的DoPairing和DoOverrides函数进行修改，使之支持多个头

2025-1-14 250114-146100001
==================
在DoPairing函数中，暂时规避ERROR_DEVICE_SEEN_BEFORE错误

2025-1-14 250114-146000001
==================
修改250113-145900001中的bug

2025-1-13 250113-145900001
==================
取消固定一个打印头，改为可灵活使用两个打印头。修改内容设计hp22mm.c, hp22mm.h，common.c, Hp22mm.java和TestHp22mm.java。仅在hp22mm.c的sPenIdx变量定义处做了注释，其他部分点位比较多没有注释。需要前后代码比对区分

2025-1-11 250111-145800001
==================
1. 在hp22mm.c的守护线程中，尝试加入pd_enable_warming，尝试持续给打印头加热
2. 修改22mm的打印头选择页面的表记。IDS0,1 -> IDS1,2; Pen0,1 -> Pen1,2

2025-1-11 250111-145700001
==================
1. 在ExportLog2Usb类中，追加了一个将hp22mm库发生的错误写入文件的操作（前后两次内容一致则不写入）
2. 在ImportDialog中，追加一个输出hp22mm库错误信息的按钮(22mm↓Log)，当该按钮点按时，显示错误信息(使用scroll_textview弹窗)。并且，如果插入U盘，则将文件写入U盘（文件名为：hp22mm_err_log.txt)
3. 在ControlTabActivity类中，当发生hp22mm库错误时，发出报警音

2025-1-10 250110-145600001
==================
22mm，R15的计算中使用的参数1(mParam[0])，最小不能小于110

2025-1-10 250110-145500001
==================
22mm，R15的计算中使用的参数1(mParam[0])，最小不能小于100

2025-1-9 250109-145400001
==================
22mm参数生成时(Hp22mm.java)，R18限定最小值不小于1

2025-1-2 250102-145300001
==================
250102-145300001的修改，调整为8(6%)

2025-1-2 250102-145200001
==================
pd_set_voltage_override设10（10%）临时改在PowerOn函数中

2024-12-31 241231-145100001
==================
22mm。monitorThread中pd_get_print_head_status返回状态2时，函数本身并不返回错误。因此不能导致尝试重启。修改为忽略函数返回值，只要状态是2（或者1）都尝试上电

2024-12-31 241231-145000001
==================
22mm，临时取消pd_disable_warming功能

2024-12-31 241231-144900001
==================
22mm中，临时在守护线程中添加读取打印头温度的功能（pd_get_temperature）。并且暂时取消1.0.122和1.0.123版本增加的电压调整的尝试

2024-12-30 241230-144800001
==================
1. 修改当装机后还没有在信息选择对话窗（打印/编辑）打开过具体的信息之前，在信息选择对话窗直接点击OK键时出现卡住死机的问题
2. 修改同样的上述条件下，直接点击删除键导致卡住死机的问题

2024-12-29 241229-144700001
==================
22mm的R8的值，临时修改为0x7FE084

2024-12-28 241228-144600001
==================
增加一个在打印过程当中显示计数器当前数值的功能，显示位置是预览区的右上角。同时在计数器打了边界的时候报警，显示提示

2024-12-27 241227-144500001
==================
pd_set_voltage_override设10（10%）

2024-12-27 241227-144400001
==================
22mm初始化PD时，尝试设置pd_set_voltage_override，暂时值为8(6%)。在MonitorThread中读取验证

2024-12-27 241227-144300001
==================
22mm的R27修改为传递DPI。300/450=0, 600/750=1,...

2024-12-27 241227-144200001
==================
1. A133版本，在MainActivity类中增加屏幕常亮的功能
2. 在参数77中增加IDS和打印头的可选功能，从而可以自由的在IDS1和IDS0，PEN1和PEN0之间切换，选择后需要重启

2024-12-24 241224-144100001
==================
【临时版本】取消241224-144000001的修改，暂时IDS_INDEX=0, PEN_INDEX=1

2024-12-24 241224-144000001
==================
【临时版本】241224-143900001基础上，PEN0做完整的初始化（包括power_on）操作

2024-12-24 241224-143900001
==================
【临时版本】241223-143800001基础上，先初始化PEN_INDEX=0，然后再初始化PEN_INDEX=1

2024-12-23 241223-143800001
==================
临时版本
1. 取消241223-143700001的修改，恢复错误时退出。
2. PEN_INDEX = 1

2024-12-23 241223-143700001
2024-12-23 241223-143600001
==================
临时版本，在22mm初始化初期的pd_get_print_head_status步骤时，如果返回错误不退出，允许继续执行。目的是在前一次初始化的最后一个步骤power_on的时候，返回状态4（INVALID）后，再次尝试初始化时pd_get_print_head_status仍然返回状态4，看看跳过去是什么结果

2024-12-23 241223-143500001
==================
借用蓝牙开启测试的位置（因为已经不需要）增加生成数据所用时间测试（可以视为一种性能测试）

2024-12-20 241220-143400001
==================
恢复IDS_INDEX = 1

2024-12-20 241220-143300001
==================
在hp22mm.c中，在上电之前先检查温度是否到位，否则等待，最多5秒

2024-12-20 241220-143200001
==================
空号，与241220-143100001完全相同，只是为了再次push一下

2024-12-20 241220-143100001
==================
1. 修改MainActivity的onCheckedChanged函数触发playClick调用的条件。A20时，控件被点按时，底层AudioBuzzer接收不到事件，因此在这里主动发出beep音；A133底层能够收到事件，因此这里就不需要主动发送beep音了，否则会听到两声
2. 64点打印头追加48点高的选项

2024-12-18 241218-143000001
==================
临时IDS_INDEX = 0

2024-12-18 241218-142900001
==================
monitorThread中暂时取消pd_set_temperature_override，可能引起14号错误

2024-12-18 241218-142800001
==================
Java_com_pd_get_print_head_status函数中，如果state和error返回错误，但是函数本身没有发挥错误，也给ERR_STRING设置错误信息

2024-12-18 241218-142700001
==================
...
3. UserGroup信息的选择
   可以通过选择信息对话窗选择相应的信息，或者点击上下滚动键选择UserGroup。当UserGroup信息被选择之后，点按下滚按钮选择该UserGroup文件所包含的所有子信息，然后通过点按上下滚动键选择具体的子信息。
