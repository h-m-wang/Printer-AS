...
// RfidScheduler类中支持的卡是原来的负担卡，这个卡维护锁值是在RfidScheduler类中进行的。但是1207卡的锁值是在N_RfidDevice中直接进行的，因此不需熬在N_RfidScheduler中进行。N_RfidScheduler中仅仅是为了管理墨袋机
public class N_RfidScheduler implements IInkScheduler {
    ...
    private final static int ADD_INK_TRY_LIMITS         = 20;       // 加墨的尝试次数
...
// H.M.Wang 2023-4-1 临时增加异常值管理，当最近5分钟内大于560的次数超过5%时，报警，停止加墨；当相邻两次取值相差50点以上的次数>30%时，报警，停止加墨
        private ArrayList<N_RfidScheduler.Level_Record> mLevelRecords;
...
// H.M.Wang 2023-4-1 临时增加异常值管理，当最近5分钟内大于560的次数超过5%时，报警，停止加墨；当相邻两次取值相差50点以上的次数>30%时，报警，停止加墨
            mLevelRecords = new ArrayList<N_RfidScheduler.Level_Record>();
            mLastLevel = -1;
            mCountGtMax = 0;
            mCountGap = 0;
            mCountError = 0;
...
// H.M.Wang 2023-4-1 临时增加异常值管理，当最近5分钟内大于560的次数超过5%时，报警，停止加墨；当相邻两次取值相差50点以上的次数>30%时，报警，停止加墨
            ...
            while(mBaginkLevels[cardIdx].mLevelRecords.size() > 0) {
                N_RfidScheduler.Level_Record lr = mBaginkLevels[cardIdx].mLevelRecords.get(0);
            ...
            mBaginkLevels[cardIdx].mLevelRecords.add(new N_RfidScheduler.Level_Record(rt, avgLevel));
