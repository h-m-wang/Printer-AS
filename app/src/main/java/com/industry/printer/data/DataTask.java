...
// H.M.Wang 2025-1-19 增加22mmx2打印头类型
		if (mTask.getNozzle() != PrinterNozzle.MESSAGE_TYPE_22MM && mTask.getNozzle() != PrinterNozzle.MESSAGE_TYPE_22MMX2 && mTask.getNozzle() != PrinterNozzle.MESSAGE_TYPE_108MM) {
...
// End of H.M.Wang 2024-3-11 追加hp22mm打印头，以生成1056点高的打印image
					...
					sysconf.getParam(SystemConfigFile.INDEX_HEAD_TYPE) == PrinterNozzle.MessageType.NOZZLE_INDEX_108MM ||
...
// H.M.Wang 2026-5-28 恢复原来保存print.bin的逻辑，修改为保存到u盘的根目录下，print<1000+>.bin
		if (Hp22mmSCManager.VIRTUAL_PRINT_MODE) {
			final ArrayList<String> usbs = ConfigPath.getMountedUsb();
			if (usbs.size() > 0) {
				BinCreater.saveBin(usbs.get(0) + "/print" + (Hp22mmSCManager.PrintCount++) + ".bin", mBuffer, mBinInfo.mBytesPerHFeed * 8 * mTask.getNozzle().mHeads);
			}
		}
...
//				if (!o.getSource()) {
				if (!o.getSource() && !((BarcodeObject) o).needRedraw()) {
...
// H.M.Wang 2024-9-20 为扫描协议7增加一个可自由选择打印头的参数，未选择的打印头数据清空。和一个反向打印的开关
	public static int WorkPens = 0x0f;
	public static boolean BackWardPrint = false;
...
// End of H.M.Wang 2026-5-11 暂时取消108MM的镜像操作
		...
		if(object.getPNozzle() == PrinterNozzle.MESSAGE_TYPE_22MM) {
			if(sysconf.getParam(14) > 0) revert = 0x10; else revert = 0x00;// 单头倒置。
		}
