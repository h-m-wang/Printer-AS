...
//    protected List< Rect > rectangles = new ArrayList<>(); // note positions do not account for quiet zones (handled in renderers)
...
//    protected List< Rect > target = new ArrayList<>();       // note positions do not account for quiet zones (handled in renderers)
//
//    /**
...
//     * allow some applications to avoid the use of {@link )}
...
//     * The specified font name needs to be registered via {@link } if you are
//     * using the {@link Java2DRenderer}. In order to set the font without registering the font with the graphics
//     * environment when using the {@link Java2DRenderer}, you may need to use {@link #} instead.
//     *
//     * <p>Use this method in combination with {@link #setFontSize(int)}.
//     *
//     * <p>Do not use this method in combination with {@link #}.
//     *
...
//                Rect firstRect = rectangles.get(i);
//                Rect secondRect = rectangles.get(j);
//                if (firstRect.left == secondRect.left && firstRect.width() == secondRect.width()) {
//                    if ((firstRect.top + firstRect.height()) == secondRect.top) {
...
public abstract class Symbol {
    ...
    protected static final int FNC3 = -3;
    protected static final int FNC4 = -4;
    protected static final int GS = -5;

    protected static final String FNC1_STRING = "\\<FNC1>";
    protected static final String FNC2_STRING = "\\<FNC2>";
    protected static final String FNC3_STRING = "\\<FNC3>";
    protected static final String FNC4_STRING = "\\<FNC4>";
    protected static final String GS_STRING = "\\<GS>";
    ...
    protected DataType inputDataType = DataType.ECI;
    protected SeparatorType separatorDataType = SeparatorType.FNC1;
    protected boolean readerInit;
    protected int default_height = 40;
    protected int quietZoneHorizontal = 0;
    protected int quietZoneVertical = 0;
...
//    protected Font font;
    ...
    public List<RectF> rectangles = new ArrayList<>(); // note positions do not account for quiet zones (handled in renderers)
    protected List< TextBox > texts = new ArrayList<>();                 // note positions do not account for quiet zones (handled in renderers)
    public List< Hexagon > hexagons = new ArrayList<>();              // note positions do not account for quiet zones (handled in renderers)
    public List< RectF > target = new ArrayList<>();       // note positions do not account for quiet zones (handled in renderers)
    ...
    public void setSeparatorType(SeparatorType separatorType) {
        ...
        separatorDataType = separatorType;
    }
...
//    }
    ...
    public void setContent(String data) {
        ...
        switch (inputDataType) {
            case GS1:
                content = Gs1.verify(data, separatorDataType == SeparatorType.FNC1 ? FNC1_STRING : GS_STRING);
                readable = data.replace('[', '(').replace(']', ')');
                break;
            case HIBC:
                content = hibcProcess(data);
    ...
    protected static int[] toBytes(String s, Charset charset, int... suffix) {
        ...
        byte[] fnc3 = FNC3_STRING.getBytes(charset);
        byte[] fnc4 = FNC4_STRING.getBytes(charset);
        byte[] gs = GS_STRING.getBytes(charset);
     ...
     * @return the module width to use for the specified original module width
     */
    protected float getModuleWidth(int originalWidth) {
        return originalWidth;
    }
    ...
    protected void mergeVerticalBlocks() {
        ...
        for (int i = rectangles.size() - 1; i >= 0; i--) {
            RectF rect1 = rectangles.get(i);
            for (int j = i - 1; j >= 0; j--) {
                RectF rect2 = rectangles.get(j);
                if (roughlyEqual(rect1.top, rect2.top + rect2.height())) {
                    // rect2 is in the segment of rectangles for the row directly above rect1
                    if (roughlyEqual(rect1.left, rect2.left) && roughlyEqual(rect1.width(), rect2.width())) {
                        // we've found a match; merge the rectangles
                        rect2.bottom += rect1.height();
//                        rect2.set(rect1.left,rect1.top,rect1.right,rect1.bottom + rect1.height());
//                        rectangles.set(i, rect2);
                        rectangles.remove(i);
                        break;
                    }
