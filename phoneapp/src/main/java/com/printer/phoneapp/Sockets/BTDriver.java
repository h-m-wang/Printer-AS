...
public class BTDriver {
    private static final String TAG = BTDriver.class.getSimpleName();
    ...
    // 蓝牙状态变化系统广播接收器，在本程序中主要用来接收打开蓝牙功能时的系统广播
    private BroadcastReceiver mStateChangeBroadcaster = new BroadcastReceiver() {
        @Override
    ...
    private boolean mStateChangeReceiverRegisterred = false;

    private void registerStateChangeBroadcaster() {
        try {
            if(null != mContext && !mStateChangeReceiverRegisterred) {
                mContext.registerReceiver(mStateChangeBroadcaster, new IntentFilter(BluetoothAdapter.ACTION_STATE_CHANGED));
                mStateChangeReceiverRegisterred = true;
            }
        } catch (Exception e) {
    ...
    private void unregisterStateChangeBroadcaster() {
        try {
            if(null != mContext && mStateChangeReceiverRegisterred) {
                mStateChangeReceiverRegisterred = false;
                mContext.unregisterReceiver(mStateChangeBroadcaster);
            }
        } catch (Exception e) {
            Log.e(TAG, e.getClass().getSimpleName() + ": " + e.getMessage());
        }
    ...
    public void finishEnabling() {
        Log.d(TAG, "Finished Enabling");
        unregisterStateChangeBroadcaster();
    }
    ...
    protected static BTDriver mBTDriver = null;
    public BTDriver(Context ctx) {
        mContext = ctx;
...
//        getBondedDevices();
    ...
    public static String getPropString(int properties) {
