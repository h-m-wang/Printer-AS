...
// 恢复mutex功能
// 1.0.200 2026-3-23
// RunningState增加初始化为VALID，否则会出现开机加压后首次读取为INVALID而触发报警
// 1.0.199 2026-3-21
// 重新整理1.0.195之后的版本，取消mutex，保留1.0.197和1.0.198的修改
// 1.0.198 2026-3-20
// 在monitorThread函数中，处于Air_Pump_State == AIR_STATE_PUMPING 及Air_Pump_State == AIR_STATE_LAUNCH_PUMP 状态时，不执行后续的ids及pd的处理，一面出现uart访问中的ids与pd的竞争
// 1.0.197 2026-3-20
// (1) 在monitorThread函数中，区分处理print_head_status.print_head_state == PH_STATE_POWERED_OFF 和 PH_STATE_PRESENT，当 PH_STATE_PRESENT时，先power_off一下再power_on
// (2) 在monitorThread函数中，当print_head_status.print_head_state == PH_STATE_POWERED_OFF 时，返回PS1(i)错误；PH_STATE_PRESENT时返回PS2(i)错误
// (3) 在monitorThread函数中，当握手协议出现错误时，分别返回HS1(i)错误和HS2(i)错误
// 1.0.196 2026-3-5
...
int RunningState[3] = {STATE_VALID,STATE_VALID,STATE_VALID};             // [0]: IDS状态； [1]: PEN0状态； [2]: PEN1状态
...
//                secure_sec[i] += POLL_SEC;
                ...
                PDResult_t pd_r = pd_get_print_head_status(PD_INSTANCE, penIndexs[i], &print_head_status);
                if(pd_r != PD_OK || print_head_status.print_head_state != PH_STATE_POWERED_ON || print_head_status.print_head_error != PH_NO_ERROR) {
                    sprintf(ERR_STRING, "ERROR: pd_get_print_head_status(PenIdx=%d) failed: %s.  PH state: %s  PH error: %d %s\n", penIndexs[i],
                            pd_get_error_description(pd_r),
                            ph_state_description(print_head_status.print_head_state),
...
// H.M.Wang 2026-2-3 正式版时带重新上电，测试版时不带重新上电。平时保持在正式版上
                ...
                if(print_head_status.print_head_state == PH_STATE_PRESENT) {
// H.M.Wang 2026-2-3 正式版时带重新上电，测试版时不带重新上电。平时保持在正式版上
                    sprintf(ERR_STRING1, "PS2(%d)", i);
                    pd_check_ph("pd_power_off", pd_power_off(PD_INSTANCE, penIndexs[i]), penIndexs[i]);       // H.M.Wang 2025-12-3 暂时禁止重新上电 2026-1-29 恢复重新上电 2026-1-31 再次暂时禁止
                    pd_check_ph("pd_power_on", pd_power_on(PD_INSTANCE, penIndexs[i]), penIndexs[i]);       // H.M.Wang 2025-12-3 暂时禁止重新上电 2026-1-29 恢复重新上电 2026-1-31 再次暂时禁止
                }
